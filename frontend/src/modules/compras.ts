...
export async function renderCompras(container?: HTMLElement) {
  ...
  try {
    await _loadComprasPage(target);
  } catch (err: any) {
    target.innerHTML = `<div class="p-8 text-center" style="color:#EF4444"><i class="fas fa-circle-exclamation mr-2"></i>${(window as any).esc(err.response?.message || err.message)}</div>`;
  }
}
  ...
  c.innerHTML = `
    ...
    <div class="flex flex-wrap items-center justify-between gap-3 mb-5">
      <div>
        ...
        <p class="text-sm" style="color:#6B7280">Gestión de compras a proveedores, facturas de proveedor, kardex e inventarios sin firma electrónica.</p>
      </div>
      ${(window as any).can('canWrite') ? `
        <div class="flex gap-2">
          ...
          <button class="btn btn-primary" id="btn-new-purchase"><i class="fas fa-plus"></i> Nueva Factura de Compra</button>
        </div>` : ''}
    </div>

    <div class="mb-4 p-3 rounded-xl flex items-center justify-between text-xs" style="background:#F0F9FF;border:1px solid #BAE6FD;color:#0369A1">
      <div><i class="fas fa-info-circle mr-1.5 text-sky-600"></i>¿Necesitas emitir un <strong>Documento Soporte Electrónico (DS)</strong> a un sujeto no obligado a facturar? Accede desde <strong>Contabilidad e Impuestos</strong>.</div>
      <button class="font-bold underline text-sky-800 hover:text-sky-950 cursor-pointer" onclick="navigate('doc-soporte')">Ir a Documento Soporte <i class="fas fa-arrow-right ml-1"></i></button>
    </div>
...
async function openPurchaseForm(invoiceId: string | null = null, onDone: any = null, preloadedImportId: string | null = null, noteConfig: any = null) {
  ...
  (window as any).__poAccountsCache = accounts;
  ...
  if (invoiceId) {
    ...
    try {
      ...
      const noteTypeLabel = noteConfig.type === 'ND' ? 'Nota Débito' : (noteConfig.type === 'NDS' ? 'Nota de Ajuste a Documento Soporte' : 'Nota Crédito');
      const invNumStr = originalInvoice.number || noteConfig.originalInvoiceNum || '';
      const cufeStr = origCufe || 'No disponible';
      const defaultNotes = `${noteTypeLabel} referente al documento N° ${invNumStr}. CUFE/CUDS: ${cufeStr}`;
  ...
  const userDefaultWhId = currentUser?.default_warehouse_id || '';
  const defaultWarehouseId = inv?.warehouse_id || (!inv && (userDefaultWhId || (warehouses.length === 1 ? warehouses[0].id : '')));
  ...
  // Filtrar tipos de comprobantes contables válidos (excluyendo Documento Soporte y sus notas, los cuales corresponden a su módulo dedicado)
  let allowedTxTypes = txTypes.filter((t: any) => {
    const code = String(t.code || '').trim().toUpperCase();
    const prefix = String(t.prefix || '').trim().toUpperCase();
    const name = String(t.name || '').trim().toLowerCase();
    if (code === 'DS' || code === 'NDS' || prefix === 'DS' || prefix === 'DSE' || prefix === 'NDS' || name.includes('soporte')) {
      return false;
    }
    return code === 'FC' || prefix === 'FC' || name.includes('compra');
  });
  if (noteConfig) {
  ...
  const nonDsTypes = txTypes.filter((t: any) => {
    const code = String(t.code || '').trim().toUpperCase();
    const prefix = String(t.prefix || '').trim().toUpperCase();
    const name = String(t.name || '').trim().toLowerCase();
    return !(code === 'DS' || code === 'NDS' || prefix === 'DS' || prefix === 'DSE' || prefix === 'NDS' || name.includes('soporte'));
  });
  const finalTypes = allowedTxTypes.length ? allowedTxTypes : nonDsTypes;

  const txTypeOptions = finalTypes
    .map((t: any) => `<option value="${(window as any).esc(t.id)}"${(inv?.tx_type_id === t.id || (!inv && !noteConfig && (t.code === 'FC' || t.prefix === 'FC'))) ? ' selected' : ''} data-code="${(window as any).esc(t.code)}">${(window as any).esc(t.prefix)} — ${(window as any).esc(t.name)}</option>`)
    .join('');
  ...
  function poUpdateSupplierTaxInfo(supplierId: string, autoSelectRules: boolean) {
    const infoWrap = document.getElementById('po-supplier-tax-info');
    if (!infoWrap) return;

    if (!supplierId) {
      infoWrap.innerHTML = '<span class="text-gray-400 text-[10px] italic"><i class="fas fa-circle-info mr-1 text-slate-300"></i>Selecciona un proveedor para cargar su configuración tributaria</span>';
      return;
    }

    const sup = suppliers.find((s: any) => s.id === supplierId);
    if (!sup) {
      infoWrap.innerHTML = '<span class="text-red-500 text-[10px]">Proveedor no encontrado</span>';
      return;
    }
    ...
    const regimeNames: Record<string, string> = {
      ...
      'RESP_IVA': 'Responsable IVA',
      'NO_RESP': 'No Responsable IVA',
      'GRAN_CONTR': 'Gran Contribuyente',
      'COMUN': 'Régimen Común',
      'SIMPLIFICADO': 'Régimen Simplificado'
    };
    const regimeLabel = regimeNames[sup.tax_regime] || sup.tax_regime || 'Régimen No Especificado';
    if (sup.tax_regime) {
      badges.push(`<span class="px-2 py-0.5 rounded bg-blue-50 text-blue-700 border border-blue-200" style="font-size:10px"><i class="fas fa-building mr-1"></i>${regimeLabel}</span>`);
    }

    // 2. Gran Contribuyente / Autorretenedor / Exento IVA
    if (sup.gc) {
      badges.push(`<span class="px-2 py-0.5 rounded bg-purple-50 text-purple-700 border border-purple-200" style="font-size:10px"><i class="fas fa-crown mr-1"></i>Gran Contribuyente</span>`);
    }
    if (sup.ar) {
      badges.push(`<span class="px-2 py-0.5 rounded bg-indigo-50 text-indigo-700 border border-indigo-200" style="font-size:10px"><i class="fas fa-hand-holding-dollar mr-1"></i>Autorretenedor</span>`);
    }
    if (sup.ei) {
      badges.push(`<span class="px-2 py-0.5 rounded bg-teal-50 text-teal-700 border border-teal-200" style="font-size:10px"><i class="fas fa-percent mr-1"></i>Exento IVA</span>`);
    }

    // 3. Exenciones de retenciones o tarifas configuradas
    const isExemptRenta = sup.ar || sup.tax_regime === 'SIMPLE';
    
    if (isExemptRenta) {
      badges.push(`<span class="px-2 py-0.5 rounded bg-emerald-50 text-emerald-700 border border-emerald-200 font-semibold" style="font-size:10px"><i class="fas fa-shield-halved mr-1"></i>Exento ReteFuente (${sup.ar ? 'Autorretenedor' : 'Régimen Simple'})</span>`);
    } else if (sup.prf > 0) {
      badges.push(`<span class="px-2 py-0.5 rounded bg-amber-50 text-amber-700 border border-amber-200" style="font-size:10px"><i class="fas fa-percent mr-1"></i>ReteFuente: ${sup.prf}%</span>`);
    } else {
      badges.push(`<span class="px-2 py-0.5 rounded bg-slate-100 text-slate-500 border border-slate-200" style="font-size:10px"><i class="fas fa-circle-minus mr-1"></i>Sin ReteFuente</span>`);
    }

    if (sup.pi > 0) {
      badges.push(`<span class="px-2 py-0.5 rounded bg-amber-50 text-amber-700 border border-amber-200" style="font-size:10px"><i class="fas fa-percent mr-1"></i>ReteICA: ${sup.pi}%</span>`);
    } else {
      badges.push(`<span class="px-2 py-0.5 rounded bg-slate-100 text-slate-500 border border-slate-200" style="font-size:10px"><i class="fas fa-circle-minus mr-1"></i>Sin ReteICA</span>`);
    }

    if (sup.piv > 0) {
      badges.push(`<span class="px-2 py-0.5 rounded bg-amber-50 text-amber-700 border border-amber-200" style="font-size:10px"><i class="fas fa-percent mr-1"></i>ReteIVA: ${sup.piv}%</span>`);
    } else {
      badges.push(`<span class="px-2 py-0.5 rounded bg-slate-100 text-slate-500 border border-slate-200" style="font-size:10px"><i class="fas fa-circle-minus mr-1"></i>Sin ReteIVA</span>`);
    }

    infoWrap.innerHTML = badges.join('');

    // Auto-configuración de selectores de retenciones
    if (autoSelectRules) {
      const rentaSelect = document.getElementById('po-hdr-ret-rule-renta') as HTMLSelectElement;
      const icaSelect = document.getElementById('po-hdr-ret-rule-ica') as HTMLSelectElement;
      const ivaSelect = document.getElementById('po-hdr-ret-rule-iva') as HTMLSelectElement;

      // ReteFuente
      if (rentaSelect) {
        if (isExemptRenta) {
          rentaSelect.value = ''; // Sin retención
        } else if (sup.prf > 0 && (window as any).__poRetRulesCache) {
          const rule = (window as any).__poRetRulesCache.find((r: any) => retKindOfRule(r) === 'renta' && Math.abs(Number(r.rate) - Number(sup.prf)) < 0.001);
          rentaSelect.value = rule ? rule.id : '';
        } else {
          rentaSelect.value = '';
        }
      }
      ...
      // ReteICA
      if (icaSelect) {
        if (sup.pi > 0 && (window as any).__poRetRulesCache) {
          const rule = (window as any).__poRetRulesCache.find((r: any) => retKindOfRule(r) === 'ica' && Math.abs(Number(r.rate) - Number(sup.pi)) < 0.001);
          icaSelect.value = rule ? rule.id : '';
        } else {
          icaSelect.value = '';
        }
      }
      ...
      // ReteIVA
      if (ivaSelect) {
        if (sup.piv > 0 && (window as any).__poRetRulesCache) {
          const rule = (window as any).__poRetRulesCache.find((r: any) => retKindOfRule(r) === 'iva' && Math.abs(Number(r.rate) - Number(sup.piv)) < 0.001);
          ivaSelect.value = rule ? rule.id : '';
        } else {
          ivaSelect.value = '';
        }
      }
  ...
  (window as any).poUpdateSupplierTaxInfo = poUpdateSupplierTaxInfo;
  ...
  const formHtml = `
    <div class="space-y-4 text-sm" style="color:#374151">
      ...
      <div class="rounded-xl p-3 mb-2 flex items-center gap-3" style="background:#FFF7ED;border:1px solid #FFEDD5">
        ...
        <div class="flex-1">
          ...
          <select id="po-dian-concept" class="form-input po-compact-inp w-full" style="border-color:#FDBA74; max-width:400px">
            ...
            <option value="2">2 - Anulación de factura de compra</option>
        ...
        <div class="grid gap-2" style="grid-template-columns:160px 175px 175px 1fr">
          ...
          <div>
            ...
            <select id="po-warehouse" class="form-input po-compact-inp" onchange="window.poRecalcLine(0)">
              ...
              ${warehouses.map(w => `<option value="${(window as any).esc(w.id)}"${w.id === defaultWarehouseId ? ' selected' : ''}>${(window as any).esc(w.name)}</option>`).join('')}
            </select>
          </div>
          <!-- Referencia Proveedor -->
          <div id="po-supplier-ref-wrap">
            <label class="po-hdr-label">Ref. factura proveedor <span style="color:#EF4444">*</span></label>
      ...
      <div class="flex gap-2 items-center">
        <div style="width: 175px;" class="flex-shrink-0">
          <select id="po-search-type-filter" class="form-input" style="border-color:#FDBA74; font-size:12px; font-weight:600; color:#C46516; height:38px">
        ...
        <div class="relative flex-1">
          ...
          <input id="po-prod-search-global" class="form-input"
                 ...
                 autocomplete="off"
                 placeholder="Buscar por nombre, código o cuenta contable de nivel 5...">
      ...
      <div class="border rounded-xl overflow-hidden" style="border-color:#E5E7EB">
        ...
        <div class="flex items-center justify-between px-4 py-2 flex-wrap gap-2" style="background:#FFF8F0;border-bottom:1px solid #E5E7EB">
          <span class="text-sm font-semibold" style="color:#0D2137"><i class="fas fa-boxes mr-1"></i> Artículos / Servicios Comprados <span id="po-items-counter" style="margin-left:6px;font-size:11px;font-weight:700;color:#9A3412;background:#FFEEDD;border-radius:999px;padding:2px 9px">0 líneas · 0 unds</span></span>
  ...
  if (typeof (window as any).openDocumentTab === 'function') {
    const tabKey = invoiceId ? `editar-compra-${invoiceId}` : 'nueva-compra';
    const tabTitle = invoiceId ? 'Editar Compra' : 'Nueva Compra';
    (window as any).openDocumentTab(tabKey, tabTitle, 'fa-cart-flatbed', formHtml, footer, () => {
      ...
      <div style="background:#fff;border-radius:18px;width:100%;max-width:520px;box-shadow:0 24px 60px rgba(0,0,0,.22);overflow:hidden;animation:poQAPfadeIn .15s ease">
        ...
        <div style="padding:20px;display:grid;grid-template-columns:140px 1fr;gap:12px">
          ...
          <div>
            ...
            <input id="po-qap-code" class="form-input" 
              value="${productCfg.auto_code ? '[AUTO-GENERADO]' : ''}" 
              placeholder="P-001" autocomplete="off"
              style="text-transform:uppercase;font-family:monospace"
              oninput="this.value=this.value.toUpperCase()"
              ${productCfg.auto_code ? 'disabled style="background:#F3F4F6;color:#9CA3AF"' : ''}
            >
          </div>
          <!-- Nombre -->
          <div>
      ...
      try {
        if (productCfg.auto_code) {
          ...
          while (dupFound) {
            ...
            const dup = await (window as any).pb.list('products', { filter: `code="${(window as any).pb.escapeFilterValue(code)}"`, perPage: 1 });
          ...
          await (window as any).API.setSetting('product_config_v1', JSON.stringify(cfg));
        } else {
          code = ((document.getElementById('po-qap-code') as HTMLInputElement)?.value || '').trim().toUpperCase();
  ...
  initPoSupplierSearch();
  poUpdateSupplierTaxInfo(inv?.supplier_id || '', false);
  ...
  (window as any).addPoLine = function(prod: any = null, preloadedLine: any = null) {
    ...
    const accountId   = prod?.account_id || preloadedLine?.account_id || '';
    const isDirectExpense = !!accountId;
    const productId   = isDirectExpense ? '' : (prod?.id   || preloadedLine?.product_id || '');
    let productCode = prod?.code || '';
    if (isDirectExpense && !productCode) {
      const aObj = ((window as any).__poAccountsCache || []).find((a: any) => a.id === accountId);
      if (aObj) productCode = aObj.code;
    }
    ...
    let lineRetRuleId = preloadedLine?.ret_rule_id || '';
    if (!preloadedLine && (window as any).__poRetMode === 'line' && (window as any).__poRetRulesCache) {
      const supplierId = (document.getElementById('po-supplier') as HTMLInputElement)?.value;
      if (supplierId) {
        const sup = suppliers.find((s: any) => s.id === supplierId);
        if (sup) {
          const isExemptRenta = sup.ar || sup.tax_regime === 'SIMPLE';
          if (!isExemptRenta && sup.prf > 0) {
            const rule = (window as any).__poRetRulesCache.find((r: any) => r.concept === 'RETERENTA' && Math.abs(Number(r.rate) - Number(sup.prf)) < 0.001);
            if (rule) {
              lineRetRuleId = rule.id;
            }
          }
        }
      }
    ...
    tr.innerHTML = `
      <td>
        <div class="flex flex-col">
          ...
          <input type="hidden" id="pol-acct-id-${idx}" value="${(window as any).esc(accountId)}">
          ${isDirectExpense ? `<input type="text" class="form-input text-xs mt-1 py-0.5" id="pol-desc-inp-${idx}" placeholder="Descripción del servicio..." value="${(window as any).esc(preloadedLine?.description || '')}" oninput="document.getElementById('po-row-${idx}').dataset.comment=this.value; if(typeof (window as any).poSaveTempState === 'function') (window as any).poSaveTempState(null)">` : ''}
        </div>
      </td>
      ...
      <td class="po-ret-col" style="display:none">
        <select id="pol-ret-rule-${idx}" class="form-input text-xs py-1 w-full" onchange="window.poRecalcLine(${idx})">
          ${retRuleOptions(withholdingRules, lineRetRuleId)}
        </select>
      </td>
  ...
  function initPoGlobalProductSearch() {
    ...
    const handleSearch = () => {
      ...
      const typeFilter = (document.getElementById('po-search-type-filter') as HTMLSelectElement)?.value || 'BIEN';
      
      let filteredProducts = [];
      if (typeFilter === 'BIEN') {
        filteredProducts = products.filter((p: any) => p.type === 'BIEN' && `${p.name} ${p.code} ${p.ean_code || ''}`.toLowerCase().includes(q));
      } else if (typeFilter === 'SERVICIO') {
        filteredProducts = products.filter((p: any) => p.type === 'SERVICIO' && `${p.name} ${p.code} ${p.ean_code || ''}`.toLowerCase().includes(q));
      } else {
        filteredProducts = products.filter((p: any) => `${p.name} ${p.code} ${p.ean_code || ''}`.toLowerCase().includes(q));
      }

      let filteredAccounts = [];
      if (typeFilter === 'SERVICIO' || typeFilter === 'ALL') {
        const allAccounts = (window as any).__poAccountsCache || [];
        filteredAccounts = allAccounts
          .filter((a: any) => a.active && Number(a.level) === 5 && `${a.code} ${a.name}`.toLowerCase().includes(q))
          .map((a: any) => ({
            id: '',
            code: a.code,
            name: `[Gasto] ${a.code} — ${a.name}`,
            type: 'SERVICIO',
            iva_rate: 0,
            cost_price: 0,
            isAccount: true,
      ...
      const combined = [...filteredProducts, ...filteredAccounts].slice(0, 40);
    ...
    document.getElementById('po-search-type-filter')?.addEventListener('change', handleSearch);
  ...
  if (existingLines.length) {
    existingLines.forEach((l: any) => {
      if (l.account_id) {
        const aObj = accounts.find((a: any) => a.id === l.account_id);
        if (aObj) l._name = `[Gasto] ${aObj.code} — ${aObj.name}`;
      } else {
...
async function savePurchaseDraftWrapper(invoiceId: string | null, onDone: any = null, inv: any = null, noteConfig: any = null) {
  ...
  try {
    ...
    if (!isDianDoc && !supplierRef.trim()) {
      throw new Error('La referencia de factura de proveedor (documento cruce) es obligatoria.');
    }
    ...
    tableRows.forEach((row: any, lineIdx: number) => {
      ...
      const acctId = (document.getElementById(`pol-acct-id-${idx}`) as HTMLInputElement)?.value || '';
      ...
      if (!prodId && !acctId) throw new Error(`Fila ${lineIdx + 1}: selecciona un producto o cuenta contable.`);
...
function poRecalcLine(targetIdx: number = 0) {
  ...
  const itemsCounterEl = document.getElementById('po-items-counter');
  if (itemsCounterEl) {
  ...
  tableRows.forEach((row: any) => {
    ...
    if (prodId || acctId) {
      ...
      if (prodId) {
        ...
        const aObj = (window as any).__poAccountsCache?.find((a: any) => a.id === acctId);
        if (aObj) name = `[Gasto] ${aObj.code} — ${aObj.name}`;
      }
      lines.push({
        product_id: prodId || null,
        account_id: acctId || null,
...
function restorePurchaseFormState(state: any) {
  ...
  if (supplierHidden && supplierSearch) {
    ...
    if (typeof (window as any).poUpdateSupplierTaxInfo === 'function') {
      (window as any).poUpdateSupplierTaxInfo(state.supplier_id || '', false);
    }
  }
  ...
  if (Array.isArray(state.lines)) {
    state.lines.forEach((l: any) => {
      if (l.account_id) {
        const aObj = (window as any).__poAccountsCache?.find((a: any) => a.id === l.account_id);
        if (aObj) {
          const fakeProd = {
            id: '',
            code: aObj.code,
            name: `[Gasto] ${aObj.code} — ${aObj.name}`,
            type: 'SERVICIO',
          ...
          (window as any).addPoLine(fakeProd, l);
        }
      } else {
        const pObj = (window as any).__poProductsCache?.find((p: any) => p.id === l.product_id);
        if (pObj) {
          (window as any).addPoLine(pObj, l);
        }
      }
    });
  }
