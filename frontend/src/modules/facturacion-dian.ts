...
async function renderFacturacionDIAN(c: HTMLElement) {
  ...
  try {
    ...
    const [resolutions, docs, txs, invoices, purchases, inmoInvoices] = await Promise.all([
      ...
      pb.listAll('transactions', { sort: '-date,-id', filter: 'status="active" || status="draft"', expand: 'tx_type_id,third_party_id' }).catch((err: any) => { loadErrors.push(`transacciones: ${err.message}`); return []; }),
      ...
      pb.listAll('inmo_invoices', { fields: 'id,tx_id,total' }).catch((err: any) => { loadErrors.push(`inmobiliarias: ${err.message}`); return []; })
    ]);
    ...
    inmoInvoices.forEach((ii: any) => { if (ii.tx_id) { totalMap.set(ii.tx_id, ii.total); invoiceIdMap.set(ii.tx_id, ii.id); } });
    ...
    // Filter transactions to only those matching DIAN Resolutions or Real Estate (IA)
    const signableTxs = txs.filter((t: any) => {
      ...
      return activePrefixes.has(prefix) || activeDocTypes.has(code) || code === 'IA';
    });
    ...
    // Default current month (Hora Colombia)
    const currentMonth = (window as any).todayStr().slice(0, 7);
    ...
    c.innerHTML = `
      <div class="flex flex-wrap items-center justify-between gap-3 mb-5">
        <div>
          <h3 class="text-lg font-bold" style="color:var(--text-strong)">Validación de documentos DIAN</h3>
      ...
      <div class="bg-white rounded-2xl border overflow-hidden" style="border-color:#F0F0F0">
        <div class="overflow-x-auto">
          <table class="data-table" id="dian-table" data-sort-col="0" data-sort-dir="desc">
            <thead>
              <tr>
                <th>Comprobante</th>
                <th>Fecha</th>
    ...
    const renderGridAndKPIs = () => {
      ...
      // Sort by invoice/document number descending by default
      filtered.sort((a, b) => b.number.localeCompare(a.number, undefined, { numeric: true, sensitivity: 'base' }));
      ...
      if (tableBody) {
        ...
        const tbl = $('#dian-table') as HTMLTableElement;
