...
async function _loadLicenciasData(container: HTMLElement): Promise<void> {
  ...
  const MODULE_META: Record<string, {
    ...
    contabilidad: {
      ...
      pages:       ['Consulta Tx', 'Reportes', 'Cierre Contable', 'Validación DIAN', 'Exógena', 'Utilidades'],
    },
    comercial: {
      label:       'Comercial',
      icon:        'fa-store',
    ...
    niif: {
      label:       'Normas Internacionales NIIF',
      icon:        'fa-calculator',
      color:       '#7F7CFF',
      description: 'Gestión NIIF: políticas contables, arrendamientos NIIF 16 y estados financieros comparativos.',
      pages:       ['Gestión NIIF (IFRS)'],
    },
    activos_fijos: {
      label:       'Activos Fijos',
      icon:        'fa-boxes-stacked',
      color:       '#10B981',
      description: 'Módulo de propiedad, planta y equipo con depreciación dual contable y fiscal.',
      pages:       ['Catálogo de Activos', 'Categorías y Mapeo', 'Depreciación Mensual', 'Inventario QR'],
    },
  };
...
async function toggleLicense(moduleKey: string, enabled: boolean): Promise<void> {
  ...
  if (toggle) {
    toggle.checked = !enabled; // Revertir visualmente mientras confirma
  }

  const alertText = enabled
    ? `¿Estás seguro de que deseas <strong>habilitar</strong> el módulo <strong>${moduleKey}</strong>? Esto activará sus funciones para todos los usuarios.`
    : `¿Estás seguro de que deseas <strong>deshabilitar</strong> el módulo <strong>${moduleKey}</strong>?<br><br><span style="color:#EF4444;font-weight:700;"><i class="fas fa-triangle-exclamation"></i> ADVERTENCIA:</span> Esto ocultará las funciones del módulo y bloqueará el acceso a la API para todos los usuarios activos en tiempo real.`;
