...
function defaultPOSConfig() {
  return {
    ...
    special: {
      ...
      default_customer_id: '',
      print_logo: true,
      enable_dian_button: true,
      keyboard_mode: false,
      ticket_title: 'DOCUMENTO EQUIVALENTE ELECTRÓNICO POS\n(Tiquete de máquina registradora con sistema P.O.S.)',
    }
  };
}
...
async function getPOSConfig() {
  try {
    ...
    const def = defaultPOSConfig();
    return {
      operational: { ...def.operational, ...parsed.operational },
      accounting: {
        ...def.accounting,
        ...(parsed.accounting || {}),
        accounts: { ...def.accounting.accounts, ...(parsed.accounting?.accounts || {}) }
      },
      special: { ...def.special, ...parsed.special }
    };
  } catch {
      ...
      <div class="rounded-xl border p-4" style="border-color:#E5E7EB;background:#FCFCFD">
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-4 text-sm">
          ...
          <div class="form-group mb-0 col-span-2">
            <label class="form-label">Título impreso en Tiquete POS</label>
            <textarea id="pos-cfg-ticket-title" class="form-input w-full" rows="2" style="background:#fff;color:#0D2137;" placeholder="Título que se imprime arriba en el tiquete...">${(window as any).esc(cfg.special?.ticket_title || 'DOCUMENTO EQUIVALENTE ELECTRÓNICO POS\n(Tiquete de máquina registradora con sistema P.O.S.)')}</textarea>
          </div>
        </div>
      </div>
    </div>
    ...
    document.getElementById('btn-pos-cfg-save')?.addEventListener('click', async () => {
      ...
      const newCfg = {
        ...
        special: {
          ...
          keyboard_mode: (document.getElementById('pos-cfg-keyboard-mode') as HTMLInputElement)?.checked,
          ticket_title: (document.getElementById('pos-cfg-ticket-title') as HTMLTextAreaElement)?.value || '',
        }
      };
...
let posDiscountType: 'percent' | 'flat' = 'percent';
...
export async function renderPOS(container: HTMLElement) {
  ...
  const canSeeCfg = ['admin', 'administrador', 'contador', 'superadmin'].includes(userRole);
...
window.loadPOSInterface = async function() {
  ...
  try {
    ...
    const [prods, thirds, whs] = await Promise.all([
      ...
      (window as any).API.getTerceros({ type: 'CLIENTE' }),
      (window as any).API.getWarehouses(true, activeShift?.branch_id || ''),
    ]);

    posProducts = prods;
    posCustomers = thirds;

    // Filtrar bodegas permitidas por el usuario
    const user = (window as any).pb.currentUser;
    let allowedWhs = whs;
    if (user?.allowed_warehouses && user.allowed_warehouses.length > 0) {
      allowedWhs = whs.filter((w: any) => user.allowed_warehouses.includes(w.id));
    }
    posWarehouses = allowedWhs;
    ...
    // Bodega por defecto
    const userDefaultWhId = user?.default_warehouse_id || "";
    if (userDefaultWhId && posWarehouses.some((w: any) => w.id === userDefaultWhId)) {
      selectedWarehouseId = userDefaultWhId;
    } else {
        ...
        <div class="lg:col-span-5 flex flex-col rounded-2xl border pos-cart-container" style="border-color:#E5E7EB;background:#FCFCFD;overflow:hidden;height: 100%">
          ...
          <div class="flex justify-between items-center border-b px-5 pt-4 pb-3 flex-shrink-0" style="border-color:#E5E7EB">
            ...
            <div class="flex items-center gap-1.5 flex-wrap">
              ...
              ${['admin','administrador','contador','superadmin'].includes((window as any).pb.currentUser?.role) ? `
              <button class="btn btn-outline btn-sm cursor-pointer rounded-lg" title="Promociones y Ofertas" onclick="window.openPOSPromotionsModal()" style="color:#10B981;border-color:#10B981">
          ...
          <div class="flex-grow overflow-y-auto px-5 py-2 space-y-3" id="pos-cart-body" style="height: 0; flex-grow: 1; min-height: 0;">
            <!-- Items del carrito -->
          </div>
            ...
            <div class="grid grid-cols-2 gap-3 text-xs border-b pb-2 mb-2" style="border-color:#E5E7EB">
              ...
              <div>
                <div class="flex justify-between items-center mb-1">
                  <label class="text-[10px] uppercase font-bold block" style="color:#6B7280">Descuento</label>
                  <select id="pos-cart-discount-type" onchange="window.posUpdateDiscountFreight()" class="text-[10px] font-bold border-none bg-transparent text-gray-500 cursor-pointer p-0 focus:ring-0 outline-none" style="height:auto; width:auto; background-image:none; padding-right:0;">
                    <option value="percent" ${posDiscountType === 'percent' ? 'selected' : ''}>%</option>
                    <option value="flat" ${posDiscountType === 'flat' ? 'selected' : ''}>$</option>
                  </select>
                </div>
                <input type="number" id="pos-cart-discount-input" class="form-input w-full text-xs py-1 px-2 font-bold" min="0" value="${posDiscountValue}" oninput="window.posUpdateDiscountFreight()" style="background:#fff;color:#0D2137;height:28px">
              </div>` : ''}
    ...
    if (posConfig?.special?.keyboard_mode) {
      const searchInp = document.getElementById('pos-search-product') as HTMLInputElement;
      const resultsDiv = document.getElementById('pos-search-product-results');
        ...
        searchInput.addEventListener('keydown', async (ev) => {
          if (ev.key === 'Enter') {
            ...
            if (resultsDiv && resultsDiv.style.display !== 'none') {
              // Dejar que initKeyboardAutocomplete maneje la selección
              return;
            }
...
window.filterPosProducts = function() {
  ...
  if (posConfig?.special?.keyboard_mode && query) {
    if (resultsDiv) {
      const filtered = posProducts.filter(p => `${p.name} ${p.code} ${p.ean_code || ''}`.toLowerCase().includes(query));
      if (!filtered.length) {
        resultsDiv.innerHTML = '<div class="px-3 py-2 text-xs text-gray-400">Sin coincidencias</div>';
      ...
      resultsDiv.innerHTML = filtered.slice(0, 30).map(p => {
        ...
        return `
          <button type="button" class="w-full text-left px-3 py-2 text-xs border-none bg-white hover:bg-gray-100 cursor-pointer block ${isBlocked ? 'opacity-55 cursor-not-allowed' : ''}"
                  data-id="${p.id}"
                  data-name="${(window as any).esc(p.name)}"
                  onmousedown="event.preventDefault()"
                  ${isBlocked ? 'disabled' : `onclick="window.onKeyboardSelectProduct('${p.id}')"`}>
            <div class="font-bold text-gray-800">${(window as any).esc(p.name)}</div>
            <div class="text-[10px] text-gray-500">Cód: ${p.code} | Stock: ${p.type === 'SERVICIO' ? 'Servicio' : p.stock} | Precio: ${(window as any).fmt(price)}</div>
          </button>
        `;
      }).join('');
      resultsDiv.style.display = 'block';
...
window.onKeyboardSelectProduct = function(productId: string) {
  ...
  const allowPriceEdit = !!posConfig?.special?.allow_price_edit || posConfig?.special?.price_source === 'free';

  const html = `
    <div class="space-y-4 text-sm" style="color:#374151">
      <div class="p-3 rounded-xl bg-blue-50 border border-blue-100 text-blue-800 font-medium">
        Agregar al carrito: <strong>${(window as any).esc(prod.name)}</strong>
      </div>
      <div class="grid grid-cols-2 gap-4">
        <div class="form-group">
          <label class="form-label font-bold block mb-1">Cantidad</label>
          <input type="number" id="kbd-qty-inp" class="form-input w-full font-bold text-lg text-center" min="1" step="1" value="1" style="background:#fff;color:#0D2137">
        </div>
        <div class="form-group">
          <label class="form-label font-bold block mb-1">Precio Unitario</label>
          <input type="number" id="kbd-price-inp" class="form-input w-full font-bold text-lg text-right" min="0" step="50" value="${price}" ${allowPriceEdit ? '' : 'disabled'} style="background:#fff;color:#0D2137">
        </div>
      </div>
    </div>
  `;
  ...
  const footer = `
    <button class="btn btn-outline" onclick="window.closeKbdPrompt()">Cancelar</button>
    <button class="btn btn-primary" id="btn-kbd-confirm" onclick="window.confirmKbdProductAdd('${prod.id}')">Agregar (Enter)</button>
  `;

  (window as any).openModal(`Agregar Producto`, html, footer, false);

  // Focus quantity field automatically
  setTimeout(() => {
    const qtyInp = document.getElementById('kbd-qty-inp') as HTMLInputElement;
    if (qtyInp) {
      qtyInp.focus();
      qtyInp.select();
      
      qtyInp.addEventListener('keydown', (ev) => {
        if (ev.key === 'Enter') {
          ev.preventDefault();
          if (allowPriceEdit) {
            const priceInp = document.getElementById('kbd-price-inp') as HTMLInputElement;
            if (priceInp) {
              priceInp.focus();
              priceInp.select();
            }
          } else {
            window.confirmKbdProductAdd(prod.id);
          }
        }
      });
    }
    ...
    if (allowPriceEdit) {
      const priceInp = document.getElementById('kbd-price-inp') as HTMLInputElement;
      if (priceInp) {
        priceInp.addEventListener('keydown', (ev) => {
          if (ev.key === 'Enter') {
            ev.preventDefault();
            window.confirmKbdProductAdd(prod.id);
          }
...
window.closeKbdPrompt = function() {
  (window as any).closeModal();
  setTimeout(() => {
    const searchInput = document.getElementById('pos-search-product') as HTMLInputElement;
    if (searchInput) {
...
window.confirmKbdProductAdd = function(productId: string) {
  const prod = posProducts.find(p => p.id === productId);
  if (!prod) return;

  const qty = parseFloat((document.getElementById('kbd-qty-inp') as HTMLInputElement)?.value || '0');
  const price = parseFloat((document.getElementById('kbd-price-inp') as HTMLInputElement)?.value || '0');

  if (Number.isNaN(qty) || qty <= 0) {
    (window as any).showToast('Cantidad no válida', 'warning');
    return;
  }
  ...
  if (prod.type === 'BIEN' && !allowNegative && currentQty + qty > prod.stock) {
    (window as any).showToast(`Existencias insuficientes. Stock disponible: ${prod.stock}`, 'warning');
    return;
  }

  if (inCart) {
    inCart.qty += qty;
    inCart.sales_price = price;
  } else {
    posCart.push({
      id: prod.id,
      code: prod.code,
  ...
  (window as any).showToast(`Agregado al carrito: ${prod.name} (x${qty})`, 'info', 1500);
  window.renderPOSCart();
  window.closeKbdPrompt();
};
...
window.renderPOSCart = function() {
  ...
  if (!posCart.length) {
    ...
    if (discTypeSelect) discTypeSelect.value = 'percent';
  ...
  body.innerHTML = posCart.map(item => {
    ...
    const prod = posProducts.find(p => p.id === item.id);
    const stockLabelVal = item.type === 'SERVICIO' ? 'SERVICIO' : `${prod ? prod.stock : 0} DISP`;
    const stockCartHtml = item.type === 'SERVICIO'
      ? ''
      : `<span>| Stock: </span><span class="cursor-pointer hover:underline text-blue-600 font-bold" onclick="(window as any).showStockBreakdownModal('${item.id}', '${(window as any).esc(item.name)}')">${stockLabelVal} <i class="fas fa-circle-info text-[9px]"></i></span>`;

    const ivaLabel = includesIva
      ? `<span class="text-orange-500 font-bold">(IVA incl. ${item.iva_rate}%)</span> ${stockCartHtml}`
      : `<span>| IVA: ${item.iva_rate}%</span> ${stockCartHtml}`;
  ...
  const discountAmount = posDiscountType === 'percent'
    ? Math.round(subtotal * (posDiscountValue / 100) * 100) / 100
    : Math.min(subtotal, Math.round(posDiscountValue * 100) / 100);
...
window.posUpdateDiscountFreight = function() {
  ...
  if (discTypeSelect) {
    posDiscountType = discTypeSelect.value as 'percent' | 'flat';
  }
  if (discInput) {
    const val = parseFloat(discInput.value);
    if (posDiscountType === 'percent') {
      posDiscountValue = Number.isNaN(val) ? 0 : Math.max(0, Math.min(100, val));
    } else {
      posDiscountValue = Number.isNaN(val) ? 0 : Math.max(0, val);
    }
  }
  if (freightInput) {
...
window.openArqueoPOSModal = async function() {
  ...
  try {
    const totals = await window.calculatePOSShiftTotals(activeShift.id);
    const cashSales = totals.cashSales;
    const cashRecaudos = totals.cashRecaudos;
    const cashEgresos = totals.cashEgresos;
    const bankRecaudos = totals.bankRecaudos || 0;
    const bankEgresos = totals.bankEgresos || 0;
    ...
    const bodyHtml = `
      <div class="space-y-5 text-sm" style="color:#374151">
        <div class="text-center p-4 rounded-xl mb-3" style="background:#F3F4F6">
          <span class="text-xs text-gray-500 block uppercase font-bold">Total Esperado en Efectivo Físico</span>
        ...
        <div class="grid grid-cols-2 gap-4">
          ...
          <div><div class="text-xs text-gray-500">Ventas en Efectivo (+)</div><div class="font-bold text-gray-800">${(window as any).fmt(cashSales)}</div></div>
          <div><div class="text-xs text-gray-500">Recaudos en Efectivo (+)</div><div class="font-bold text-green-600">${(window as any).fmt(cashRecaudos)}</div></div>
          <div><div class="text-xs text-gray-500">Egresos en Efectivo (-)</div><div class="font-bold text-red-600">${(window as any).fmt(cashEgresos)}</div></div>
        </div>

        <div class="p-3.5 rounded-xl space-y-2" style="background:#EFF6FF;border:1px solid #BFDBFE">
          <span class="text-[10px] text-blue-800 block uppercase font-bold tracking-wider">Movimientos por Banco (No afectan caja física)</span>
          <div class="grid grid-cols-2 gap-4 text-xs">
            <div><div class="text-gray-500">Recaudos por Transf. (+)</div><div class="font-bold text-blue-700">${(window as any).fmt(bankRecaudos)}</div></div>
            <div><div class="text-gray-500">Egresos por Transf. (-)</div><div class="font-bold text-blue-700">${(window as any).fmt(bankEgresos)}</div></div>
          </div>
        </div>
  ...
  try {
    // Recalcular
     ...
     await (window as any).pb.update('pos_shifts', activeShift.id, {
       ...
       cash_recaudos: cashRecaudos,
       cash_egresos: cashEgresos,
       bank_recaudos: bankRecaudos,
       bank_egresos: bankEgresos,
...
window.printArqueoTicket = async function(shiftId: string) {
  try {
    ...
    const totalSales = efecSales + transSales + credSales + otherSales;
    ...
    const recs = shift.cash_recaudos || 0;
    const egrs = shift.cash_egresos || 0;
    const expected = shift.cash_initial + efecSales + recs - egrs;
    ...
    let html = `
      ...
      <head>
        ...
        <style>
          ...
          body { font-family: monospace; font-size: 11px; margin: 2mm; color:#000; width: 62mm; line-height: 1.3; }
      ...
      <body>
        <div class="center bold">${(window as any).esc(compName)}</div>
        ...
        <div class="center bold">ARQUEO Y CUADRE DE CAJA</div>
        <div class="dbl-hr"></div>
        ...
        <div class="flex-between"><span>Recaudos Caja (+):</span><span>${(window as any).fmt(recs)}</span></div>
        <div class="flex-between"><span>Egresos Caja (-):</span><span>${(window as any).fmt(egrs)}</span></div>
        <div class="hr"></div>
        ...
        <div class="hr"></div>
        <div class="flex-between"><span>Ventas Transf. (Banco):</span><span>${(window as any).fmt(transSales)}</span></div>
        <div class="flex-between"><span>Recaudos Transf. (+):</span><span>${(window as any).fmt(shift.bank_recaudos || 0)}</span></div>
        <div class="flex-between"><span>Egresos Transf. (-):</span><span>${(window as any).fmt(shift.bank_egresos || 0)}</span></div>
        ...
        <div class="dbl-hr"></div>
        <div class="center bold">TOTAL DE VENTAS</div>
        <div class="hr"></div>
        <div class="flex-between"><span>No. Transacciones:</span><span>${totalInvoices}</span></div>
        <div class="flex-between"><span>Efectivo:</span><span>${(window as any).fmt(efecSales)}</span></div>
        <div class="flex-between"><span>Transferencias:</span><span>${(window as any).fmt(transSales)}</span></div>
        <div class="flex-between"><span>Crédito:</span><span>${(window as any).fmt(credSales)}</span></div>
        ${otherSales > 0 ? `<div class="flex-between"><span>Otros:</span><span>${(window as any).fmt(otherSales)}</span></div>` : ''}
        <div class="hr"></div>
        <div class="flex-between bold" style="font-size:13px"><span>TOTAL VENTAS:</span><span>${(window as any).fmt(totalSales)}</span></div>
        <div class="dbl-hr"></div>
  ...
  try {
    ...
    let html = `
      ...
      <body>
        ...
        <div class="dbl-hr"></div>
        <div class="center bold">COMPROBANTE INFORME DIARIO DE VENTAS</div>
        <div class="center">(Resolución 000055 de 2016 - DIAN)</div>
        <div class="dbl-hr"></div>
        ...
        <div class="hr"></div>
        <div class="flex-between bold"><span>TOTAL REGISTRADO:</span><span>${(window as any).fmt(grandTotal)}</span></div>
...
window.openPOSPaymentModal = async function() {
  ...
  try {
    const [sList, bList] = await Promise.all([
      ...
      (window as any).pb.listAll('bank_accounts', { filter: 'active=true', sort: 'name' })
    ]);
    sellers = sList;
    bankAccounts = bList;
  } catch (err) {
    console.warn("Fallo al obtener maestros en POS payment modal:", err);
  }
  ...
  const bodyHtml = `
    <div class="space-y-6 text-sm" style="color:#374151">
      ...
      <!-- Sección Transferencia / Banco Destino (Visible solo si método es TRANSFERENCIA) -->
      <div id="pos-pay-transfer-sec" class="space-y-4 border-t pt-4" style="border-color:#E5E7EB; display:none">
        <div>
          <label class="form-label font-bold text-gray-700 mb-1.5 block">Cuenta Bancaria Destino <span style="color:#EF4444">*</span></label>
          <select id="pos-pay-bank-account" class="form-input w-full">
            ${bankAccounts.map(b => `<option value="${(window as any).esc(b.id)}">${(window as any).esc(b.bank)} — ${(window as any).esc(b.number)} (${(window as any).esc(b.name)})</option>`).join('')}
          </select>
          <p class="text-[10px] text-gray-400 mt-1">Selecciona la cuenta destino donde el cliente transfirió el dinero.</p>
        </div>
      </div>
      ...
      <div id="pos-pay-mixed-sec" class="space-y-4 border-t pt-4" style="border-color:#E5E7EB; display:none">
        ...
        <div class="space-y-3">
          ...
          <div class="grid grid-cols-12 gap-3 items-center" id="pos-mixed-bank-row" style="display:none">
            <div class="col-span-4 text-xs font-bold text-blue-500">Banco Destino <span style="color:#EF4444">*</span></div>
            <div class="col-span-8">
              <select id="pos-mixed-bank-account" class="form-input w-full text-xs" style="height:34px">
                ${bankAccounts.map(b => `<option value="${(window as any).esc(b.id)}">${(window as any).esc(b.bank)} — ${(window as any).esc(b.number)}</option>`).join('')}
              </select>
            </div>
          </div>
          <div class="grid grid-cols-12 gap-3 items-center">
...
window.selectPosPayMethod = function(method: string) {
  ...
  const transferSec = document.getElementById('pos-pay-transfer-sec');
  
  if (cashSec) cashSec.style.display = method === 'EFECTIVO' ? 'block' : 'none';
  if (mixedSec) mixedSec.style.display = method === 'MIXTO' ? 'block' : 'none';
  if (transferSec) transferSec.style.display = method === 'TRANSFERENCIA' ? 'block' : 'none';
...
window.posMixedCalc = function() {
  ...
  const bankRow = document.getElementById('pos-mixed-bank-row');
  if (bankRow) {
    bankRow.style.display = trans > 0 ? 'grid' : 'none';
  }
...
window.confirmPOSPayment = async function() {
  ...
  if (currentPOSPayMethod === 'EFECTIVO') {
    ...
    bankAccountId = (document.getElementById('pos-pay-bank-account') as HTMLSelectElement)?.value || null;
    if (!bankAccountId) {
      (window as any).showToast('Por favor selecciona la cuenta bancaria destino.', 'warning');
      return;
    }
    ...
    if (trans > 0) {
      bankAccountId = (document.getElementById('pos-mixed-bank-account') as HTMLSelectElement)?.value || null;
      if (!bankAccountId) {
        (window as any).showToast('Por favor selecciona la cuenta bancaria para la porción de transferencia.', 'warning');
        return;
      }
    }
...
function generateStableCufe(seed: string) {
  let hash = 0;
...
window.showThermalTicketReceipt = async function(invoiceId: string, receivedCash: number, changeCash: number) {
  try {
    ...
    const rawTitle = isAccepted 
      ? 'FACTURA ELECTRÓNICA DE VENTA' 
      : (posConfig?.special?.ticket_title || 'DOCUMENTO EQUIVALENTE ELECTRÓNICO POS\n(Tiquete de máquina registradora con sistema P.O.S.)');
    const title = (window as any).esc(rawTitle).replace(/\n/g, '<br>');
    ...
    try {
      ...
      if (resolution) {
        ...
        resolutionInfoHtml = `
          <div class="text-center" style="margin-top:4px;line-height:1.2">
    ...
    let qrAndCufeHtml = '';
    const cufeVal = einvoiceDoc?.cufe || generateStableCufe(invoiceId);
    if (cufeVal) {
      qrAndCufeHtml = `
        <div>================================</div>
        ...
        <div style="word-break:break-all;text-align:center;margin-top:4px;line-height:1.1">
    ...
    let errorDianHtml = '';
    if (einvoiceDoc && (einvoiceDoc.status === 'rechazada' || (einvoiceDoc.dian_response && einvoiceDoc.dian_response.toLowerCase().includes('error')))) {
      errorDianHtml = `
        <div class="p-3 mb-3 rounded-xl border text-red-800 bg-red-50 border-red-200 text-xs">
          <div class="font-bold flex items-center gap-1.5 mb-1 text-red-900">
            <i class="fas fa-circle-exclamation text-sm"></i> Rechazado por la DIAN / Error de Emisión
          </div>
          <p class="font-medium">${(window as any).esc(einvoiceDoc.dian_response || 'Error desconocido al emitir a la DIAN.')}</p>
        </div>
      `;
    }
        ...
        <div id="pos-thermal-tirilla" class="mx-auto p-5 border shadow-inner font-mono text-[11px] leading-tight text-black max-w-[280px]" style="background:#fffff8;border-color:#e2e8f0;box-shadow:inset 0 0 10px rgba(0,0,0,0.05)">
          ${(posConfig?.special?.print_logo !== false && compLogo) ? `
            <div class="text-center" style="margin-bottom:8px">
              <img src="data:image/png;base64,${compLogo}" style="max-height:130px; max-width:250px; display:inline-block; object-fit:contain;" />
            </div>
          ` : ''}
          <div class="text-center" style="font-weight:bold">${(window as any).esc(compName)}</div>
          ...
          ${lines.map((l: any) => {
            ...
            return `
              <div style="margin-bottom:4px">
                ...
                <div style="color:#000;">Cód: ${(window as any).esc(l.expand?.product_id?.code || '—')} | IVA: ${l.iva_rate ?? 0}%</div>
                <div style="display:flex;justify-content:between">
          ...
          <div style="display:flex;justify-content:between;font-weight:bold;"><span>TOTAL:</span><span style="float:right">${(window as any).fmt(inv.payable_total ?? inv.total ?? 0)}</span></div>
          ${taxBreakdownHtml}
          <div>================================</div>
          ...
          ` : ''}
          <div>================================</div>
          ${(resolutionName && !resolutionName.toLowerCase().includes('factura de venta pos')) ? `
            <div class="text-center" style="font-weight:bold">${resolutionName}</div>
          ` : ''}
          ${resolutionDesc ? `<div class="text-center">${resolutionDesc}</div>` : ''}
          ${resolutionInfoHtml}
    ...
    `;
    if (!isAccepted && posConfig?.special?.enable_dian_button !== false) {
      footer += `
    ...
    (window as any).confirmDialog(
      ...
      async () => {
        try {
          ...
          const errMsg = err.response?.message || err.message || 'Error en comunicación';
  ...
  try {
    ...
    try {
      ...
      if (resolution) {
        ...
        const docLabel = resolution.document_type === 'FV' ? 'Resolución Facturación Electrónica' : 'Autorización Facturación POS';
        resolutionInfoHtml = `
          <div class="center" style="margin-top:4px;line-height:1.2">
            ${docLabel} No. ${resolution.resolution_number}<br>
            Fecha Res: ${rDate} | Vigencia hasta: ${eDate}<br>
            Rango: ${rPrefix}${resolution.number_from} al ${rPrefix}${resolution.number_to}
          </div>
    ...
    if (cufeVal) {
      qrAndCufeHtml = `
        ...
        <div class="center" style="word-break:break-all;margin-top:4px;line-height:1.1">
          <strong>CUFE:</strong><br>${cufeVal}
        </div>
      `;
    } else {
    ...
    printWin.document.write(`
      ...
      <body>
        ...
        <div class="dbl-hr"></div>
        ${(resolutionName && !resolutionName.toLowerCase().includes('factura de venta pos')) ? `
          <div class="center bold">${resolutionName}</div>
        ` : ''}
        ${resolutionDesc ? `<div class="center">${resolutionDesc}</div>` : ''}
        ${resolutionInfoHtml}
  ...
  try {
    ...
    (window as any).posApplyOrderToCart = async function(orderId: string) {
      ...
      try {
        ...
        // Set discount if it exists on order
        const discInput = document.getElementById('pos-cart-discount-input') as HTMLInputElement;
        const discTypeSelect = document.getElementById('pos-cart-discount-type') as HTMLSelectElement;
        if (order.discount_amount) {
          posDiscountType = 'flat';
          posDiscountValue = order.discount_amount;
          if (discTypeSelect) discTypeSelect.value = 'flat';
          if (discInput) discInput.value = String(posDiscountValue);
        } else {
          posDiscountType = 'percent';
          posDiscountValue = 0;
          if (discTypeSelect) discTypeSelect.value = 'percent';
  ...
  setTimeout(() => {
    ...
    confirmBtn?.addEventListener('click', () => {
      ...
      const heldCart = {
        ...
        discountPct: posDiscountType === 'percent' ? posDiscountValue : 0,
...
window.posResumeHeldCart = function(id: string) {
  ...
  posDiscountType = target.discountType || 'percent';
  posDiscountValue = target.discountValue || target.discountPct || 0;
  ...
  const discountInput = document.getElementById('pos-cart-discount-input') as HTMLInputElement;
  const discountTypeSelect = document.getElementById('pos-cart-discount-type') as HTMLSelectElement;
  const freightInput = document.getElementById('pos-cart-freight-input') as HTMLInputElement;
  if (discountTypeSelect) discountTypeSelect.value = posDiscountType;
  if (discountInput) discountInput.value = String(posDiscountValue);
...
// ─── REGISTRO DE MOVIMIENTOS DIRECTOS DE CAJA (EGRESOS / RECAUDOS) ───────────

window.calculatePOSShiftTotals = async function(shiftId: string) {
  const pb = (window as any).pb;

  // 1. Cargar ventas del turno en efectivo
  const res = await pb.listAll('invoices', {
    filter: `pos_shift_id="${shiftId}" && status="posted"`
  });
  
  const cashSales = res.reduce((sum: number, inv: any) => sum + (inv.payment_method === 'EFECTIVO' ? (inv.payable_total ?? inv.total) : 0), 0);

  // 2. Cargar movimientos directos (RC y CE) del turno (efectivo: 1105, bancos: 1110/1120)
  let cashRecaudos = 0;
  let cashEgresos = 0;
  let bankRecaudos = 0;
  let bankEgresos = 0;
  try {
    const cashLines = await pb.listAll('tx_lines', {
      filter: `tx_id.pos_shift_id="${shiftId}" && tx_id.status="active" && (account_id.code ~ "1105" || account_id.code ~ "1110" || account_id.code ~ "1120")`,
      expand: 'tx_id,tx_id.tx_type_id,account_id'
    });

    for (const l of cashLines) {
      const code = l.expand?.tx_id?.expand?.tx_type_id?.code || '';
      const accCode = l.expand?.account_id?.code || '';
      const isCash = accCode.startsWith('1105');

      if (code === 'RC') {
        const val = (l.debit || 0) - (l.credit || 0);
        if (isCash) cashRecaudos += val;
        else bankRecaudos += val;
      } else if (code === 'CE') {
        const val = (l.credit || 0) - (l.debit || 0);
        if (isCash) cashEgresos += val;
        else bankEgresos += val;
      }
    }
  } catch(err) {
    console.error("Error al calcular movimientos directos de caja:", err);
  }

  const expectedCash = activeShift.cash_initial + cashSales + cashRecaudos - cashEgresos;
...
window.openPOSCashMovementsModal = async function() {
  if (!activeShift) return;

  const bodyHtml = `
    <div class="space-y-4 text-sm text-gray-700" style="font-family:'Segoe UI',sans-serif">
      <!-- Tipo de Movimiento -->
      <div class="form-group">
        <label class="block text-xs font-bold text-gray-500 uppercase tracking-wider mb-1">Tipo de Movimiento</label>
        <div class="grid grid-cols-2 gap-3">
          <button type="button" id="btn-mov-type-egreso" class="btn btn-outline py-2.5 font-bold text-xs justify-center border-red-300 text-red-600 bg-red-50/20" onclick="window.changePOSCashMovementType('egreso')">
            <i class="fas fa-arrow-up-from-bracket mr-1.5"></i>EGRESO (GASTO)
          </button>
          <button type="button" id="btn-mov-type-recaudo" class="btn btn-outline py-2.5 font-bold text-xs justify-center border-gray-200 text-gray-500" onclick="window.changePOSCashMovementType('recaudo')">
            <i class="fas fa-arrow-down-to-bracket mr-1.5"></i>RECAUDO (INGRESO)
          </button>
        </div>
        <input type="hidden" id="mov-type-value" value="egreso">
      </div>

      <!-- Concepto -->
      <div class="form-group">
        <label class="block text-xs font-bold text-gray-500 uppercase tracking-wider mb-1">Concepto Preconfigurado <span style="color:#EF4444">*</span></label>
        <select id="mov-concepto" class="form-input w-full" style="background:#fff;color:#0D2137">
      ...
      <!-- Método de Pago -->
      <div class="form-group">
        <label class="block text-xs font-bold text-gray-500 uppercase tracking-wider mb-1">Método de Pago / Origen del dinero</label>
        <select id="mov-metodo-pago" class="form-input w-full" style="background:#fff;color:#0D2137" onchange="window.onPOSMovMethodChange()">
          <option value="efectivo">Efectivo (Afecta Caja Física)</option>
          <option value="transferencia">Transferencia / Consignación (Banco)</option>
        </select>
      </div>

      <!-- Selección de Banco -->
      <div class="form-group" id="mov-bank-select-wrap" style="display:none">
        <label class="block text-xs font-bold text-gray-500 uppercase tracking-wider mb-1">Cuenta de Banco / Destino <span style="color:#EF4444">*</span></label>
        <select id="mov-cuenta-bancaria" class="form-input w-full" style="background:#fff;color:#0D2137">
      ...
      <!-- Monto -->
      <div class="form-group">
        <label class="block text-xs font-bold text-gray-500 uppercase tracking-wider mb-1">Monto (COP) <span style="color:#EF4444">*</span></label>
        <div class="relative">
          <div class="absolute inset-y-0 left-0 pl-3 flex items-center pointer-events-none text-gray-400 font-bold">$</div>
          <input type="number" id="mov-monto" class="form-input w-full pl-8 font-black text-lg" min="1" step="50" placeholder="0" style="background:#fff;color:#0D2137">
        </div>
      </div>

      <!-- Tercero / Beneficiario -->
      <div class="form-group">
        <label class="block text-xs font-bold text-gray-500 uppercase tracking-wider mb-1">Tercero / Beneficiario <span style="color:#EF4444">*</span></label>
        <div id="mov-third-wrap" class="relative">
          ...
          <input id="mov-third-search" class="form-input pl-8 py-2 text-sm w-full" autocomplete="off" placeholder="Buscar por NIT o nombre..." style="background:#fff;color:#0D2137">
          <input id="mov-third-hidden" type="hidden" value="">
          <div id="mov-third-results" style="display:none;position:absolute;left:0;right:0;top:calc(100% + 4px);max-height:180px;overflow:auto;background:#fff;border:1px solid #E5E7EB;border-radius:10px;box-shadow:0 10px 25px rgba(0,0,0,.12);z-index:99"></div>
        </div>
      </div>

      <!-- Observaciones -->
      <div class="form-group">
        <label class="block text-xs font-bold text-gray-500 uppercase tracking-wider mb-1">Observaciones / Descripción</label>
        <textarea id="mov-obs" class="form-input w-full text-xs" rows="2" placeholder="Ej: Pago de papelería para oficina, servicio de fletes..." style="background:#fff;color:#0D2137"></textarea>
      </div>
    </div>
  `;
  ...
  const footerHtml = `
    <button class="btn btn-outline" onclick="closeModal()">Cancelar</button>
    <button class="btn btn-primary" id="btn-save-mov" onclick="window.savePOSCashMovement()">
      <i class="fas fa-save mr-2"></i>REGISTRAR MOVIMIENTO
    </button>
  `;

  (window as any).openModal('Registro de Egresos y Recaudos Directos', bodyHtml, footerHtml, false);

  // Inicializar tipo y autocomplete de terceros
  setTimeout(async () => {
    window.changePOSCashMovementType('egreso');
    
    // Cargar cuentas bancarias activas
    try {
      const pb = (window as any).pb;
      const bankAccounts = await pb.listAll('bank_accounts', { expand: 'account_id', filter: 'active=true', sort: 'name' });
      const bankSelect = document.getElementById('mov-cuenta-bancaria') as HTMLSelectElement;
      if (bankSelect) {
        bankSelect.innerHTML = '<option value="">Selecciona la cuenta de banco...</option>' + bankAccounts.map((b: any) => `
          <option value="${b.expand?.account_id?.id || b.account_id}">
            ${(window as any).esc(b.name)} (${(window as any).esc(b.number || '')}) - ${(window as any).esc(b.expand?.account_id?.code || '')}
          </option>
        `).join('');
      }
    } catch(err) {
      console.warn("Fallo al obtener cuentas bancarias:", err);
    }
    
    // Autocomplete para terceros
    const wrap = document.getElementById('mov-third-wrap');
    const input = document.getElementById('mov-third-search') as HTMLInputElement;
    const hidden = document.getElementById('mov-third-hidden') as HTMLInputElement;
    const results = document.getElementById('mov-third-results');
    if (!wrap || !input || !hidden || !results) return;
    ...
    const consumer = defaultId 
      ...
      : posCustomers.find(c => c.doc_number === '222222222' || c.nit === '222222222' || c.name.toLowerCase().includes('consumidor'));
    if (consumer) {
      hidden.value = consumer.id;
      input.value = `${consumer.doc_number || consumer.nit || ''} - ${consumer.name}`;
    }
    ...
    input.addEventListener('input', () => {
      ...
      timer = setTimeout(async () => {
        ...
        if (!val) return;
        try {
          const res = await (window as any).pb.list('third_parties', {
            filter: `active=true && (name~"${(window as any).pb.escapeFilterValue(val)}" || doc_number~"${(window as any).pb.escapeFilterValue(val)}")`,
            perPage: 20
          });
          if (!res.items.length) {
            results.innerHTML = '<div class="px-3 py-2 text-xs text-gray-400">Sin coincidencias</div>';
          ...
          results.innerHTML = res.items.map((t: any) => `
            <button type="button" class="w-full text-left px-3 py-2 text-xs border-none bg-white hover:bg-gray-100 cursor-pointer block text-gray-800"
                    onclick="window.selectPOSCashMovementThird('${(window as any).esc(t.id)}', '${(window as any).esc(t.doc_number || '')} - ${(window as any).esc(t.name)}')">
              <div class="font-bold">${(window as any).esc(t.name)}</div>
              <div class="text-[10px] text-gray-500">Doc: ${t.doc_number || 'S/N'}</div>
            </button>
          `).join('');
          results.style.display = 'block';
        } catch(_) {}
    ...
    input.addEventListener('focus', async () => {
      try {
        const res = await (window as any).pb.list('third_parties', { filter: 'active=true', perPage: 20, sort: 'name' });
        results.innerHTML = res.items.map((t: any) => `
          <button type="button" class="w-full text-left px-3 py-2 text-xs border-none bg-white hover:bg-gray-100 cursor-pointer block text-gray-800"
                  onclick="window.selectPOSCashMovementThird('${(window as any).esc(t.id)}', '${(window as any).esc(t.doc_number || '')} - ${(window as any).esc(t.name)}')">
            <div class="font-bold">${(window as any).esc(t.name)}</div>
...
window.onPOSMovMethodChange = function() {
  const method = (document.getElementById('mov-metodo-pago') as HTMLSelectElement)?.value;
  const wrap = document.getElementById('mov-bank-select-wrap');
  if (wrap) wrap.style.display = method === 'transferencia' ? 'block' : 'none';
};

window.changePOSCashMovementType = async function(type: 'egreso' | 'recaudo') {
  const isEgreso = type === 'egreso';
  const valInput = document.getElementById('mov-type-value') as HTMLInputElement;
  if (valInput) valInput.value = type;

  const btnEgreso = document.getElementById('btn-mov-type-egreso');
  const btnRecaudo = document.getElementById('btn-mov-type-recaudo');

  if (isEgreso) {
    btnEgreso?.classList.add('border-red-300', 'text-red-600', 'bg-red-50/20');
    btnEgreso?.classList.remove('border-gray-200', 'text-gray-500');
    btnRecaudo?.classList.add('border-gray-200', 'text-gray-500');
    btnRecaudo?.classList.remove('border-green-300', 'text-green-600', 'bg-green-50/20');
  } else {
    btnRecaudo?.classList.add('border-green-300', 'text-green-600', 'bg-green-50/20');
    btnRecaudo?.classList.remove('border-gray-200', 'text-gray-500');
    btnEgreso?.classList.add('border-gray-200', 'text-gray-500');
    btnEgreso?.classList.remove('border-red-300', 'text-red-600', 'bg-red-50/20');
  }

  const select = document.getElementById('mov-concepto') as HTMLSelectElement;
  if (select) {
    select.innerHTML = '<option value="">Cargando conceptos...</option>';
    try {
      const concepts = await (window as any).pb.listAll('cash_concepts', {
        filter: `active=true && type="${type}"`,
        sort: 'name'
      });
      if (!concepts.length) {
        select.innerHTML = '<option value="">No hay conceptos configurados</option>';
        return;
      }
      select.innerHTML = concepts.map((c: any) => `
        <option value="${c.id}" data-account="${c.account_id}">${(window as any).esc(c.name)}</option>
      `).join('');
    } catch(err: any) {
...
window.selectPOSCashMovementThird = function(id: string, name: string) {
  const hidden = document.getElementById('mov-third-hidden') as HTMLInputElement;
  const input = document.getElementById('mov-third-search') as HTMLInputElement;
  const results = document.getElementById('mov-third-results');
  if (hidden && input) {
...
window.savePOSCashMovement = async function() {
  const type = (document.getElementById('mov-type-value') as HTMLInputElement)?.value || 'egreso';
  const conceptSelect = document.getElementById('mov-concepto') as HTMLSelectElement;
  const montoInput = document.getElementById('mov-monto') as HTMLInputElement;
  const thirdHidden = document.getElementById('mov-third-hidden') as HTMLInputElement;
  const obsInput = document.getElementById('mov-obs') as HTMLInputElement;

  const conceptId = conceptSelect?.value;
  const conceptOpt = conceptSelect?.options[conceptSelect.selectedIndex];
  const conceptName = conceptOpt?.text || '';
  const conceptAccountId = conceptOpt?.dataset?.account || '';
  const monto = parseFloat(montoInput?.value || '0');
  const thirdPartyId = thirdHidden?.value || '';
  const obs = obsInput?.value.trim() || '';

  if (!conceptId || !conceptAccountId) {
    (window as any).showToast('Por favor selecciona un concepto válido.', 'warning');
    return;
  }
  if (Number.isNaN(monto) || monto <= 0) {
    (window as any).showToast('Por favor ingresa un monto mayor a 0.', 'warning');
    return;
  }
  if (!thirdPartyId) {
    (window as any).showToast('Por favor selecciona un tercero / beneficiario.', 'warning');
    return;
  }

  const btn = document.getElementById('btn-save-mov') as HTMLButtonElement;
  if (btn) {
    btn.disabled = true;
  ...
  try {
    const pb = (window as any).pb;
    const method = (document.getElementById('mov-metodo-pago') as HTMLSelectElement)?.value || 'efectivo';
    
    let targetAccountId = "";
    
    if (method === 'efectivo') {
      // Resolver cuenta de caja real de forma robusta
      let cashAccId = "";
      try {
      ...
      if (!cashAccId) {
        try {
          let acc = await pb.list('accounts', { filter: 'code="110505"', perPage: 1 });
          if (acc.items.length) {
            cashAccId = acc.items[0].id;
          } else {
            acc = await pb.list('accounts', { filter: 'code ~ "1105"', sort: 'code', perPage: 1 });
            if (acc.items.length) cashAccId = acc.items[0].id;
          }
        } catch (_) {}
      }
      if (!cashAccId) {
        (window as any).showToast('No se encontró ninguna cuenta de caja configurada en el plan de cuentas (código 1105).', 'error');
        if (btn) {
          btn.disabled = false;
      ...
      const bankSelect = document.getElementById('mov-cuenta-bancaria') as HTMLSelectElement;
      targetAccountId = bankSelect?.value;
      if (!targetAccountId) {
        (window as any).showToast('Por favor selecciona una cuenta de banco.', 'warning');
        if (btn) {
          btn.disabled = false;
          btn.textContent = 'Registrar Movimiento';
        }
    ...
    const isEgreso = type === 'egreso';
    const typeCode = isEgreso ? 'CE' : 'RC';

    const txTypeRes = await pb.listAll('transaction_types', { filter: `code="${typeCode}"` });
    if (!txTypeRes.length) throw new Error(`No se encontró el tipo de transacción: ${typeCode}`);

    const txLines = [
      {
        account_id: conceptAccountId,
        debit: isEgreso ? monto : 0,
        credit: isEgreso ? 0 : monto,
        description: `Concepto: ${conceptName}. ${obs}`.trim(),
        third_party_id: thirdPartyId
      },
      {
        account_id: targetAccountId,
        debit: isEgreso ? 0 : monto,
        credit: isEgreso ? monto : 0,
        description: `${method === 'efectivo' ? 'Caja' : 'Banco'} por movimiento: ${conceptName}. ${obs}`.trim(),
        third_party_id: thirdPartyId
      }
    ];

    const tx = await (window as any).API.createTransaction({
      tx_type_id: txTypeRes[0].id,
      number: 'AUTO',
      date: (window as any).nowStr().slice(0, 10),
      description: `Movimiento Caja: ${conceptName}. ${obs}`.trim(),
      third_party_id: thirdPartyId,
      status: 'active',
      pos_shift_id: activeShift.id,
      branch_id: activeShift.branch_id || null
    ...
    (window as any).showToast('Movimiento de caja registrado correctamente', 'success');

    // Cerrar modal actual
    (window as any).closeModal();
    
    // Abrir modal de éxito con opciones de impresión
    const successHtml = `
      <div class="space-y-4 text-center text-sm" style="color:#374151">
        <div class="w-12 h-12 bg-green-100 text-green-600 rounded-full flex items-center justify-center text-xl mx-auto mb-2">
          <i class="fas fa-check-double"></i>
        </div>
        <h4 class="font-extrabold text-base text-gray-900">¡Movimiento Registrado con Éxito!</h4>
        <p class="text-xs text-gray-500">Se han guardado todos los datos contables en la transacción <strong>${tx.number}</strong>.</p>
        
        <div class="bg-gray-50 p-3.5 rounded-xl border max-w-sm mx-auto text-left space-y-1.5" style="border-color:#E2E8F0">
          <div class="flex justify-between"><span>Tipo:</span><span class="font-bold">${isEgreso ? 'Egreso (Salida)' : 'Recaudo (Entrada)'}</span></div>
          <div class="flex justify-between"><span>Concepto:</span><span class="font-bold">${conceptName}</span></div>
          <div class="flex justify-between"><span>Monto:</span><span class="font-bold">${(window as any).fmt(monto)}</span></div>
        </div>

        <div class="grid grid-cols-2 gap-2 max-w-xs mx-auto pt-3">
          <button class="btn btn-secondary justify-center text-xs py-2" onclick="window.printCashMovementTicket('${tx.id}')">
            <i class="fas fa-print mr-2"></i> Imprimir Tirilla
          </button>
          <button class="btn btn-primary justify-center text-xs py-2" onclick="window.printCashMovementLetter('${tx.id}')">
    ...
    const successFooter = `
      <button class="btn btn-outline justify-center w-full text-xs" onclick="closeModal()">Finalizar y Salir</button>
    `;

    (window as any).openModal(`Movimiento de Caja Certificado`, successHtml, successFooter, false);
...
window.printCashMovementTicket = async function(txId: string) {
  try {
    const pb = (window as any).pb;
    const tx = await pb.get('transactions', txId, { expand: 'third_party_id,user_id,tx_type_id' });
    ...
    const isEgreso = tx.expand?.tx_type_id?.code === 'CE';
    const title = isEgreso ? 'COMPROBANTE DE EGRESO' : 'RECIBO DE CAJA';
    const amountLine = lines.find((l: any) => l.account_id === lines[0].account_id);
    const amount = amountLine ? (isEgreso ? amountLine.debit : amountLine.credit) : 0;
    ...
    let html = `
      <html>
      <head>
        <title>Soporte de Caja - ${tx.number}</title>
        <style>
          @page { size: 80mm auto; margin: 0; }
          body { font-family: monospace; font-size: 11px; margin: 2mm; color:#000; width: 62mm; line-height: 1.3; }
          .center { text-align: center; }
      ...
      <body>
        ...
        <div class="dbl-hr"></div>
        <div class="center bold">${title}</div>
        <div class="center bold">No. ${tx.number}</div>
        <div class="dbl-hr"></div>
        <div><strong>Fecha:</strong> ${tx.date || tx.created.slice(0, 10)}</div>
        <div><strong>Tercero:</strong> ${(window as any).esc(tx.expand?.third_party_id?.name || 'Consumidor Final')}</div>
        <div><strong>Doc/NIT:</strong> ${(window as any).esc(tx.expand?.third_party_id?.doc_number || '')}</div>
        <div><strong>Cajero:</strong> ${(window as any).esc(tx.expand?.user_id?.name || '—')}</div>
        <div class="hr"></div>
        <div class="bold">DESCRIPCIÓN DEL CONCEPTO:</div>
        <div>${(window as any).esc(tx.description)}</div>
        <div class="hr"></div>
        <div class="flex-between bold" style="font-size:12px">
          <span>VALOR:</span>
          <span>${(window as any).fmt(amount)}</span>
        </div>
        <div class="dbl-hr" style="margin-top:20px"></div>
        <br>
        <div class="center">_______________________________</div>
        <div class="center">Firma Responsable</div>
        <script>
          window.onload = function() { window.print(); setTimeout(function(){ window.close(); }, 500); }
        </script>
      </body>
    ...
    (window as any).showToast('Error al imprimir ticket: ' + err.message, 'error');
  }
};

window.printCashMovementLetter = async function(txId: string) {
  try {
    const pb = (window as any).pb;
    const tx = await pb.get('transactions', txId, { expand: 'third_party_id,user_id,tx_type_id' });
    const lines = await pb.listAll('tx_lines', { filter: `tx_id="${txId}"`, expand: 'account_id' });

    const [compName, compNit, compAddress, compPhone, compEmail, compLegal] = await Promise.all([
      (window as any).API.getSetting('company_name').catch(() => 'GRAVY S.A.S'),
      (window as any).API.getSetting('company_nit').catch(() => '901.442.115-3'),
      (window as any).API.getSetting('company_address').catch(() => ''),
      (window as any).API.getSetting('company_phone').catch(() => ''),
      (window as any).API.getSetting('company_email').catch(() => ''),
      (window as any).API.getSetting('representante_legal_name').catch(() => ''),
    ]);
    ...
    const amountLine = lines.find((l: any) => l.account_id === lines[0].account_id);
    const amount = amountLine ? (isEgreso ? amountLine.debit : amountLine.credit) : 0;
    const tNombre = tx.expand?.third_party_id?.name || 'Consumidor Final';
    const tDoc = tx.expand?.third_party_id?.doc_number || '';
    const tDir = tx.expand?.third_party_id?.address || '';
    const tPhone = tx.expand?.third_party_id?.phone || '';

    const tableRows = lines.map((l: any) => `
      <tr style="border-bottom:1px solid #E5E7EB">
        <td style="padding:10px;font-family:monospace;font-size:11px">${(window as any).esc(l.expand?.account_id?.code || '')} - ${(window as any).esc(l.expand?.account_id?.name || '')}</td>
        <td style="padding:10px;text-align:right">${l.debit > 0 ? (window as any).fmt(l.debit) : ''}</td>
        <td style="padding:10px;text-align:right">${l.credit > 0 ? (window as any).fmt(l.credit) : ''}</td>
      </tr>
    `).join('');
    ...
    let html = `
      ...
      <html>
      <head>
        <title>${title} - ${tx.number}</title>
        <style>
          body { font-family: 'Segoe UI', Arial, sans-serif; font-size: 13px; color: #374151; margin: 0; padding: 20px; }
          .page { width: 216mm; margin: 0 auto; background: #fff; border: 1px solid #E5E7EB; border-radius: 8px; padding: 30px; }
          .flex-between { display: flex; justify-content: space-between; }
          table { width: 100%; border-collapse: collapse; margin-top: 15px; }
          th, td { border: 1px solid #E5E7EB; padding: 10px; text-align: left; }
          th { background: #F9FAFB; font-weight: bold; }
          .no-print { text-align: center; margin-top: 20px; }
          .no-print button { padding: 9px 24px; background: #1E40AF; color: #fff; border: none; border-radius: 6px; cursor: pointer; font-weight: 600; }
          @media print { .no-print { display: none; } body { padding: 0; } .page { border: none; padding: 0; } }
        </style>
      </head>
      <body>
        <div class="page">
          <div class="flex-between" style="margin-bottom: 20px;">
            <div>
              <div style="font-size: 18px; font-weight: 800; color: #0D2137;">${(window as any).esc(compName)}</div>
              <div style="font-size: 11px; color: #6B7280;">NIT: ${compNit}</div>
              <div style="font-size: 11px; color: #6B7280;">${compAddress}</div>
              <div style="font-size: 11px; color: #6B7280;">Tel: ${compPhone} ${compEmail ? ' | ' + compEmail : ''}</div>
            </div>
            <div style="background: ${isEgreso ? '#B91C1C' : '#1D6F42'}; color: #fff; padding: 10px 20px; border-radius: 8px; text-align: center;">
              <div style="font-size: 10px; font-weight: 800; letter-spacing: 1px;">${title}</div>
              <div style="font-size: 20px; font-weight: 900; margin-top: 2px;">No. ${tx.number}</div>
            </div>
          </div>
          <hr style="border: none; border-top: 2px solid ${isEgreso ? '#B91C1C' : '#1D6F42'}; margin-bottom: 20px;">
          
          <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 20px; margin-bottom: 20px;">
            <div style="background: #F9FAFB; padding: 12px; border-radius: 8px; border: 1px solid #E5E7EB;">
              <div style="font-size: 9px; font-weight: 700; color: #6B7280; text-transform: uppercase;">DATOS DEL TERCERO</div>
              <div style="font-weight: bold; font-size: 13px; margin-top: 4px;">${(window as any).esc(tNombre)}</div>
              <div style="margin-top: 2px;">C.C. / NIT: ${tDoc}</div>
              <div style="font-size: 11px; color: #6B7280;">${tDir}</div>
            </div>
            <div style="background: #F9FAFB; padding: 12px; border-radius: 8px; border: 1px solid #E5E7EB;">
              <div style="font-size: 9px; font-weight: 700; color: #6B7280; text-transform: uppercase;">DETALLE DEL MOVIMIENTO</div>
              <div style="margin-top: 4px;"><strong>Fecha:</strong> ${tx.date || tx.created.slice(0, 10)}</div>
              <div><strong>Cajero:</strong> ${(window as any).esc(tx.expand?.user_id?.name || '—')}</div>
            </div>
          </div>

          <div style="background: #F9FAFB; border: 1.5px solid #E5E7EB; border-radius: 8px; padding: 15px; margin-bottom: 20px; display: flex; justify-content: space-between; align-items: center;">
            <div>
              <div style="font-size: 9px; font-weight: 700; color: #6B7280; text-transform: uppercase;">VALOR REGISTRADO</div>
              <div style="font-size: 24px; font-weight: 900; color: ${isEgreso ? '#B91C1C' : '#1D6F42'}; margin-top: 2px;">${(window as any).fmt(amount)}</div>
            </div>
            <div style="max-width: 60%; text-align: right;">
              <div style="font-size: 9px; font-weight: 700; color: #6B7280; text-transform: uppercase;">DESCRIPCIÓN / OBSERVACIONES</div>
              <div style="font-size: 12px; margin-top: 4px; font-style: italic;">${(window as any).esc(tx.description)}</div>
            </div>
          </div>

          <div style="margin-bottom: 20px;">
            <div style="font-size: 10px; font-weight: 700; color: #6B7280; text-transform: uppercase; margin-bottom: 6px;">ASIENTO CONTABLE EN CONTABILIDAD</div>
            <table>
              <thead>
                <tr>
                  <th>Cuenta Contable</th>
                  <th style="text-align: right; width: 150px;">Débito</th>
                  <th style="text-align: right; width: 150px;">Crédito</th>
                </tr>
              </thead>
              <tbody>
                ${tableRows}
          ...
          <div style="display: flex; gap: 40px; justify-content: space-around; margin-top: 50px;">
            <div style="text-align: center; flex: 1;">
              <div style="border-top: 1px solid #374151; padding-top: 6px; margin-top: 40px;">
                <div style="font-size: 11px; font-weight: bold;">${(window as any).esc(tx.expand?.user_id?.name || 'Cajero')}</div>
            ...
            <div style="text-align: center; flex: 1;">
              <div style="border-top: 1px solid #374151; padding-top: 6px; margin-top: 40px;">
                ...
                <div style="font-size: 10px; color: #6B7280;">Firma Recibido / Autorizado</div>
                <div style="font-size: 9px; color: #9CA3AF;">NIT/C.C.: ${tDoc}</div>
              </div>
            </div>
          </div>

          <div style="margin-top: 30px; padding-top: 10px; border-top: 1px dashed #D1D5DB; text-align: center; font-size: 9px; color: #9CA3AF;">
            GRAVY v2.0 - Soporte de Caja Contable - Generado el ${new Date().toLocaleString('es-CO')}
          </div>
        </div>

        <div class="no-print">
          <button onclick="window.print()">Imprimir Comprobante</button>
          <button onclick="window.close()" style="margin-left: 10px; background: #E5E7EB; color: #374151;">Cerrar</button>
        </div>
      </body>
      </html>
    `;
    ...
    (window as any).showToast('Error al cargar comprobante: ' + err.message, 'error');
  }
};
