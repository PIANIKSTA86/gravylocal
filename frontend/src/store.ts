...
// --- Configuración ---
let WHATSAPP_NUMERO = '573000000000'; // Número de WhatsApp por defecto para despacho
let NOMBRE_TIENDA = 'GRAVY';
...
// --- Carga Inicial de Configuración E-commerce ---
  ...
  try {
    ...
    if (res.ok) {
      ...
      if (config.whatsapp_number) {
        WHATSAPP_NUMERO = config.whatsapp_number;
      }
      if (config.store_name) {
        NOMBRE_TIENDA = config.store_name;
      }
    }
  } catch (err) {
...
async function fetchCatalog() {
  try {
    ...
    // Sincronizar el carrito de compras con los productos activos del catálogo
    const activeProductIds = new Set(PRODUCTOS.map(p => p.id));
    const originalLength = state.carrito.length;
    state.carrito = state.carrito.filter(item => activeProductIds.has(item.id));
    if (state.carrito.length !== originalLength) {
      guardarCarrito();
    }
