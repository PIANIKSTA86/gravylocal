...
onBootstrap((e) => {
  ...
  try {
    ...
    try {
      const roleField = uca.fields.getByName("role");
      if (roleField) {
        let changed = false;
        const requiredRoles = ["cajero", "vendedor", "propietario"];
        let valuesCopy = [...roleField.values];
        requiredRoles.forEach(r => {
          if (!valuesCopy.includes(r)) {
            valuesCopy.push(r);
            changed = true;
            console.log("[GRAVY HUB] Rol " + r + " añadido a user_company_access.");
          }
        });
        if (changed) {
      ...
      console.log("[GRAVY HUB] Error al parchar roles en user_company_access: " + e);
    }
  } catch (_) {
    uca = new Collection({
      ...
      fields: [
        ...
        { name: "role",          type: "select",   required: true, values: ["admin","contador","auxiliar","cajero","auditor","viewer","vendedor","propietario"] },
  ...
  try {
    ...
    if (licenses && licenses.fields) {
      for (let i = 0; i < licenses.fields.length; i++) {
        ...
        if (licenses.fields[i].name === "enabled" && licenses.fields[i].required === true) {
          enabledField = licenses.fields[i];
        }
      }
    }
    
    // Parche dinámico para asegurar que enabled no sea required en bases de datos existentes
    if (enabledField) {
      enabledField.required = false;
      schemaChanged = true;
      console.log("[GRAVY HUB] Parcheando campo 'enabled' en licenses para required=false.");
    }
    ...
    if (moduleKeyField) {
      ...
      if (schemaChanged && moduleKeyField) {
    ...
    if (schemaChanged) {
      $app.save(licenses);
      console.log("[GRAVY HUB] Esquema de colección 'licenses' actualizado.");
    }
  } catch (_) {
    ...
    $app.save(licenses);
    console.log("[GRAVY HUB] Colección 'licenses' creada con enabled required=false.");
  }
