...
onBootstrap((e) => {
  ...
  // 1. Agregar el tipo "VENDEDOR" a la colección third_parties
  try {
    const typeField = tpCol.fields.getByName("type");
    if (typeField && !typeField.values.includes("VENDEDOR")) {
      typeField.values.push("VENDEDOR");
      $app.save(tpCol);
      console.log("[GRAVY-COMISIONES] Tipo 'VENDEDOR' agregado a third_parties.");
    }
  } catch (err) {
    console.error("[GRAVY-COMISIONES] Error al modificar campo type en third_parties:", err);
  }
