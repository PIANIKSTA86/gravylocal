...
onBootstrap((e) => {
  ...
  // ──────────────────────────────────────────────────────────
  // A. PARCHE DE ESQUEMA: licenses.module_key y licenses.enabled
  // ──────────────────────────────────────────────────────────
  try {
    const licensesCol = $app.findCollectionByNameOrId("licenses");
    let moduleKeyField = null;
    let enabledField = null;
    ...
    if (licensesCol && licensesCol.fields) {
      for (let i = 0; i < licensesCol.fields.length; i++) {
        ...
        if (licensesCol.fields[i].name === "enabled") {
          enabledField = licensesCol.fields[i];
        }
      }
    }

    if (enabledField && enabledField.required === true) {
      enabledField.required = false;
      schemaChanged = true;
      console.log("[GRAVY-CRM-LOGISTICA] Parcheando campo 'enabled' en licenses para required=false.");
    }
    ...
    if (schemaChanged) {
      ...
      console.log("[GRAVY-CRM-LOGISTICA] Esquema de colección 'licenses' actualizado localmente.");
    }
  } catch (err) {
    console.log("[GRAVY-CRM-LOGISTICA] Aviso al parchar licenses: " + err);
  }
  ...
  try {
    ...
    try {
      ...
      if (usersId) {
        fields.push({ name: "user_id", type: "relation", required: false, collectionId: usersId, cascadeDelete: false });
      }
      if (thirdPartiesId) {
        fields.push({ name: "seller_id", type: "relation", required: false, collectionId: thirdPartiesId, cascadeDelete: false });
      }
      if (salesOrdersId) {
        fields.push({ name: "sales_order_id", type: "relation", required: false, collectionId: salesOrdersId, cascadeDelete: false });
      }
      if (invoicesId) {
        fields.push({ name: "invoice_id", type: "relation", required: false, collectionId: invoicesId, cascadeDelete: false });
      }
  ...
  // ──────────────────────────────────────────────────────────
  // 1b. COLECCIÓN: crm_interactions (Bitácora de Seguimiento)
  // ──────────────────────────────────────────────────────────
  let crmInteractionsId = "";
  try {
    crmInteractionsId = $app.findCollectionByNameOrId("crm_interactions").id;
  } catch (_) {
    try {
      const crmInteractions = new Collection({
        ...
        deleteRule: deleteRule,
        fields: [
          { name: "deal_id", type: "relation", required: true, collectionId: crmDealsId, cascadeDelete: true, maxSelect: 1 },
          { name: "user_id", type: "relation", required: true, collectionId: usersId, cascadeDelete: false, maxSelect: 1 },
          { name: "type", type: "select", required: true, values: ["LLAMADA", "CORREO", "REUNION", "WHATSAPP", "COTIZACION", "OTRO"] },
          { name: "request_details", type: "text", required: true },
          { name: "response_details", type: "text", required: false },
          { name: "response_at", type: "text", required: false },
          { name: "created", type: "autodate", onCreate: true, onUpdate: false },
          { name: "updated", type: "autodate", onCreate: true, onUpdate: true }
        ]
      });
      $app.save(crmInteractions);
      crmInteractionsId = crmInteractions.id;
      console.log("[GRAVY-CRM] Colección crm_interactions creada.");
    } catch (err) {
      console.log("[GRAVY-CRM] Error al crear crm_interactions: " + err);
    }
  }
    ...
    try {
      const logisticaVehicles = new Collection({
        ...
        fields: [
          ...
          { name: "notes", type: "text", required: false },
          { name: "active", type: "bool", required: false },
          { name: "licencia_vence", type: "text", required: false },
          { name: "soat_vence", type: "text", required: false },
          { name: "tecnomecanica_vence", type: "text", required: false },
          { name: "poliza_rc_vence", type: "text", required: false },
          { name: "created", type: "autodate", onCreate: true, onUpdate: false },
          { name: "updated", type: "autodate", onCreate: true, onUpdate: true }
        ],
  ...
  // ──────────────────────────────────────────────────────────
  const collectionsToPatch = ["crm_deals", "crm_interactions", "logistica_vehicles", "logistica_deliveries", "invoices", "sales_orders", "pos_shifts"];
  collectionsToPatch.forEach(colName => {
    try {
      ...
      if (colName === "crm_deals") {
        let hasUserId = false;
        let hasSellerId = false;
        let hasSalesOrderId = false;
        let hasInvoiceId = false;

        try { hasUserId = !!col.fields.getByName("user_id"); } catch (_) {
          if (col.fields && col.fields.length) {
            for (let i = 0; i < col.fields.length; i++) {
              if (col.fields[i].name === "user_id") { hasUserId = true; break; }
            }
          }
        }
        try { hasSellerId = !!col.fields.getByName("seller_id"); } catch (_) {
          if (col.fields && col.fields.length) {
            for (let i = 0; i < col.fields.length; i++) {
              if (col.fields[i].name === "seller_id") { hasSellerId = true; break; }
            }
          }
        }
        try { hasSalesOrderId = !!col.fields.getByName("sales_order_id"); } catch (_) {
          if (col.fields && col.fields.length) {
            for (let i = 0; i < col.fields.length; i++) {
              if (col.fields[i].name === "sales_order_id") { hasSalesOrderId = true; break; }
            }
          }
        }
        try { hasInvoiceId = !!col.fields.getByName("invoice_id"); } catch (_) {
          if (col.fields && col.fields.length) {
            for (let i = 0; i < col.fields.length; i++) {
              if (col.fields[i].name === "invoice_id") { hasInvoiceId = true; break; }
            }
          }
        }

        if (!hasUserId && usersId) {
          ...
          changed = true;
          console.log("[GRAVY-MIGRATION] Campo 'user_id' agregado a crm_deals");
        }
        if (!hasSellerId && thirdPartiesId) {
          col.fields.add(new RelationField({
            name: "seller_id",
          ...
          changed = true;
          console.log("[GRAVY-MIGRATION] Campo 'seller_id' agregado a crm_deals");
        }
        if (!hasSalesOrderId && salesOrdersId) {
          col.fields.add(new RelationField({
            name: "sales_order_id",
          ...
          changed = true;
          console.log("[GRAVY-MIGRATION] Campo 'sales_order_id' agregado a crm_deals");
        }
        if (!hasInvoiceId && invoicesId) {
          col.fields.add(new RelationField({
            name: "invoice_id",
          ...
          changed = true;
          console.log("[GRAVY-MIGRATION] Campo 'invoice_id' agregado a crm_deals");
        }
      }

      if (colName === "logistica_vehicles") {
        let hasTransportista = false;
        let hasLicenciaVence = false;
        let hasSoatVence = false;
        let hasTecnomecanicaVence = false;
        let hasPolizaRcVence = false;
        ...
        hasTransportista = checkField("transportista_id");
        hasLicenciaVence = checkField("licencia_vence");
        hasSoatVence = checkField("soat_vence");
        hasTecnomecanicaVence = checkField("tecnomecanica_vence");
        hasPolizaRcVence = checkField("poliza_rc_vence");
        ...
        if (!hasLicenciaVence) {
          col.fields.add(new TextField({ name: "licencia_vence", required: false }));
          changed = true;
          console.log(`[GRAVY-MIGRATION] Campo 'licencia_vence' agregado a la coleccion ${colName}`);
        }
        if (!hasSoatVence) {
          col.fields.add(new TextField({ name: "soat_vence", required: false }));
          changed = true;
          console.log(`[GRAVY-MIGRATION] Campo 'soat_vence' agregado a la coleccion ${colName}`);
        }
        if (!hasTecnomecanicaVence) {
          col.fields.add(new TextField({ name: "tecnomecanica_vence", required: false }));
          changed = true;
          console.log(`[GRAVY-MIGRATION] Campo 'tecnomecanica_vence' agregado a la coleccion ${colName}`);
        }
        if (!hasPolizaRcVence) {
          col.fields.add(new TextField({ name: "poliza_rc_vence", required: false }));
          changed = true;
          console.log(`[GRAVY-MIGRATION] Campo 'poliza_rc_vence' agregado a la coleccion ${colName}`);
        }
      }
