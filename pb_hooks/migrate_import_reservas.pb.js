...
onBootstrap((e) => {
  ...
  let reservationsId = "";
  try {
    const col = $app.findCollectionByNameOrId("sales_reservations");
    reservationsId = col.id;
    let changed = false;
    const statusField = col.fields.find(f => f.name === "status");
    if (statusField && statusField.values) {
      if (!statusField.values.includes("por_confirmar")) { statusField.values.push("por_confirmar"); changed = true; }
      if (!statusField.values.includes("accepted")) { statusField.values.push("accepted"); changed = true; }
    }
    if (changed) {
      $app.save(col);
      console.log("[GRAVY-RESERVAS-IMPORT] Colección sales_reservations actualizada con nuevos estados por_confirmar/accepted.");
    }
  } catch (_) {
    try {
      const col = new Collection({
        ...
        fields: [
          ...
          { name: "status", type: "select", required: true, values: ["active", "partial", "completed", "released", "cancelled", "por_confirmar", "accepted"] },
