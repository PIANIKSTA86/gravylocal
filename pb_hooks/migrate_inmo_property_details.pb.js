...
onBootstrap((e) => {
    ...
    const addFieldIfMissing = (name, type, options = {}) => {
      try {
        if (!propCol.fields.getByName(name)) {
          let fieldObj;
          if (type === 'text') {
            fieldObj = new TextField({ name: name, required: false, ...options });
          } else if (type === 'number') {
            fieldObj = new NumberField({ name: name, required: false, ...options });
          } else if (type === 'bool') {
            fieldObj = new BoolField({ name: name, required: false, ...options });
          } else {
            fieldObj = new Field({ name: name, type: type, required: false, ...options });
          }
          propCol.fields.add(fieldObj);
          needsSaveProp = true;
  ...
  // 1.1 Agregar campo tax_amount a inmo_invoices si no existe
  try {
    const invCol = $app.findCollectionByNameOrId("inmo_invoices");
    if (invCol && !invCol.fields.getByName("tax_amount")) {
      invCol.fields.add(new NumberField({ name: "tax_amount", required: false, min: 0 }));
      $app.save(invCol);
      console.log("[GRAVY-INMO-DETAILS] Campo 'tax_amount' agregado a inmo_invoices.");
    }
  } catch (err) {
    console.error("[GRAVY-INMO-DETAILS] Error al verificar campo tax_amount en inmo_invoices:", err);
  }
