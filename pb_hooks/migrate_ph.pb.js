...
onBootstrap((e) => {
  ...
  try {
    ...
    const phInvoices = new Collection({
      ...
      listRule:   "@request.auth.id != ''",
      viewRule:   "@request.auth.id != ''",
      createRule: "@request.auth.collectionName = 'users' && (@request.auth.role = 'admin' || @request.auth.role = 'contador' || @request.auth.role = 'auxiliar' || @request.auth.role = 'superadmin')",
      updateRule: "@request.auth.collectionName = 'users' && (@request.auth.role = 'admin' || @request.auth.role = 'contador' || @request.auth.role = 'auxiliar' || @request.auth.role = 'superadmin')",
  ...
  // Normalizar ph_invoice_lines si ya existía: reglas sincronizadas
  try {
    const phILCol = $app.findCollectionByNameOrId('ph_invoice_lines');
    const writeRule = "@request.auth.collectionName = 'users' && (@request.auth.role = 'admin' || @request.auth.role = 'contador' || @request.auth.role = 'auxiliar' || @request.auth.role = 'superadmin')";
    let changed = false;
    if (phILCol.createRule !== writeRule) { phILCol.createRule = writeRule; changed = true; }
    if (phILCol.updateRule !== writeRule) { phILCol.updateRule = writeRule; changed = true; }
    if (!new Set(phILCol.fields.fieldNames()).has('account_code')) {
      phILCol.fields.add(new TextField({ name: 'account_code', required: false }));
      changed = true;
    }
    if (changed) {
      $app.save(phILCol);
      console.log('[GRAVY-PH] Colección ph_invoice_lines actualizada: reglas/campos sincronizados.');
    }
  } catch (err) {
  ...
  try {
    const writeRule = "@request.auth.collectionName = 'users' && (@request.auth.role = 'superadmin' || @request.auth.role = 'administrador' || @request.auth.role = 'admin' || @request.auth.role = 'contador' || @request.auth.role = 'auxiliar')";
    const writableCollections = ['ph_common_areas', 'ph_billing_concepts', 'ph_individual_charges', 'ph_properties'];
    let adjusted = 0;
