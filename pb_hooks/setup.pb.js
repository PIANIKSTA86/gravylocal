...
onBootstrap((e) => {
  ...
  const thirdParties = new Collection({
    ...
    fields: [
      { name: "type",               type: "select",  required: true,  values: ["CLIENTE","PROVEEDOR","EMPLEADO","PROPIETARIO","OTRO"] },
      { name: "doc_type",           type: "select",  required: true,  values: ["11","12","13","21","22","31","41","42","47","48","50","91","NIT","NITPE","CC","CE","TI","PAS","RC"] },
  ...
  const txLines = new Collection({
    ...
    createRule: "@request.auth.collectionName = 'users' && (@request.auth.role != 'auditor' && @request.auth.role != 'viewer')",
    updateRule: "@request.auth.collectionName = 'users' && (@request.auth.role = 'superadmin' || @request.auth.role = 'administrador' || @request.auth.role = 'admin' || @request.auth.role = 'contador' || @request.auth.role = 'auxiliar')",
    deleteRule: "@request.auth.collectionName = 'users' && (@request.auth.role = 'superadmin' || @request.auth.role = 'administrador' || @request.auth.role = 'admin' || @request.auth.role = 'contador' || @request.auth.role = 'auxiliar')",
    fields: [
  ...
  const payrollPeriods = new Collection({
    ...
    fields: [
      ...
      { name: "tx_id",     type: "relation", required: false, collectionId: transactionsId, cascadeDelete: false, maxSelect: 999 },
    ],
  });
  ...
  const einvoiceDocs = new Collection({
    ...
    fields: [
      ...
      { name: "zip_filename",  type: "text",     required: false },
    ],
  });
  ...
  const inventoryMovements = new Collection({
    ...
    fields: [
      ...
      { name: "total_qty",      type: "number",   required: false, min: 0 },
      { name: "total_cost",     type: "number",   required: false, min: 0 },
    ],
  ...
  try {
    ...
    if (!existing.has('role')) {
      usersCol.fields.add(new SelectField({
        name: "role",
        required: true,
        values: ["superadmin","admin","contador","auxiliar","cajero","auditor","viewer","vendedor","propietario"],
      }));
    } else {
      // Asegurar que superadmin, cajero, vendedor y propietario estén en la lista de valores si el campo ya existe
      const roleField = usersCol.fields.getByName("role");
      if (roleField) {
        let changed = false;
        const requiredRoles = ["superadmin", "cajero", "vendedor", "propietario"];
        let valuesCopy = [...roleField.values];
        requiredRoles.forEach(r => {
          if (!valuesCopy.includes(r)) {
            valuesCopy.push(r);
            changed = true;
            console.log("[GRAVY] Rol " + r + " añadido a users.");
          }
        });
        if (changed) {
          roleField.values = valuesCopy;
        }
      }
    }
...
onRecordCreateRequest((e) => {
  ...
  // Guardamos primero en DB. Si falla, el flujo se corta aquí y no se actualiza el consecutivo en transaction_types.
  e.next();
...
onBootstrap((e) => {
  ...
  try {
    const usersCol = $app.findCollectionByNameOrId("users");
    const listRule = "@request.auth.collectionName = 'users' && @request.auth.role = 'superadmin'";
    const viewRule = "@request.auth.collectionName = 'users' && (@request.auth.role = 'superadmin' || @request.auth.id = id)";
    const updateRule = "@request.auth.collectionName = 'users' && (@request.auth.role = 'superadmin' || (@request.auth.id = id && @request.auth.role != 'viewer'))";
    const createRule = "@request.auth.collectionName = 'users' && @request.auth.role = 'superadmin'";
    const deleteRule = "@request.auth.collectionName = 'users' && @request.auth.role = 'superadmin'";
    ...
    if (usersCol.createRule !== createRule) {
      usersCol.createRule = createRule;
      changed = true;
    }
    if (usersCol.deleteRule !== deleteRule) {
      usersCol.deleteRule = deleteRule;
      changed = true;
    }
  ...
  try {
    ...
    if (txCol.deleteRule !== targetDeleteRule) {
      ...
      $app.save(txCol);
      console.log("[GRAVY] Reglas de eliminación de transactions actualizadas (Superadmin habilitado).");
    }

    const invMovCol = $app.findCollectionByNameOrId("inventory_movements");
    if (invMovCol.deleteRule !== targetDeleteRule) {
      invMovCol.deleteRule = targetDeleteRule;
      $app.save(invMovCol);
      console.log("[GRAVY] Reglas de eliminación de inventory_movements actualizadas.");
    }

    const invMovLinesCol = $app.findCollectionByNameOrId("inventory_movement_lines");
    if (invMovLinesCol.deleteRule !== targetDeleteRule) {
      invMovLinesCol.deleteRule = targetDeleteRule;
      $app.save(invMovLinesCol);
      console.log("[GRAVY] Reglas de eliminación de inventory_movement_lines actualizadas.");
    }
  } catch (err) {
    console.log("[GRAVY] Aviso al ajustar reglas de eliminación de colecciones: " + err);
  }
});
...
onBootstrap((e) => {
  ...
  try {
    ...
    try {
      ...
      let changedTxLines = false;
      const txLinesExtra = [
        ["cross_doc_ref",  new TextField({ name: "cross_doc_ref",  required: false })],
        ["cross_doc_date", new TextField({ name: "cross_doc_date", required: false })],
        ["due_date",       new TextField({ name: "due_date",       required: false })],
      ];
      for (const [fname, fieldObj] of txLinesExtra) {
        let hasIt = false;
        try { hasIt = !!txLinesCol.fields.getByName(fname); } catch (_) { hasIt = String(txLinesCol.fields || "").includes(fname); }
        if (!hasIt) {
          txLinesCol.fields.add(fieldObj);
          changedTxLines = true;
        }
      }
      if (changedTxLines) $app.save(txLinesCol);
    } catch (err) {
      console.log("[GRAVY] Aviso migrando tx_lines: " + err);
    }
  } catch (err) {
