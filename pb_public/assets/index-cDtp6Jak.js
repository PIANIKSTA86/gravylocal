  ...
  <script>window.onload = function(){ window.print(); }<\/script>
    ...
    <div id="aging-results" class="p-8 text-center" style="color:#9CA3AF">
      <i class="fas fa-hourglass-half mr-2"></i>Selecciona filtros y pulsa Generar.
          ...
          </tr>`);for(const N of I){const L=N.expired_days<0?"#059669":N.expired_days<=30?"#D97706":"#EF4444";C.push(`<tr>
      ...
      <i class="fas fa-calendar-days mr-2"></i>Selecciona el lapso y pulsa Generar.
    </div>`;let i=[],n=null;const r=async()=>{var g,h;const b=$("#trial-results");if(!b)return;const f=getInputVal("trial-from"),d=getInputVal("trial-to"),m=getSelectVal("trial-level"),p=m==="all"?Number.POSITIVE_INFINITY:Number(m||3),_=getCheckVal("trial-show-third"),v=getCheckVal("trial-show-signatures");if(!f||!d)return showToast("Selecciona el lapso (desde y hasta).","warning");if(f>d)return showToast("La fecha Desde no puede ser mayor que Hasta.","warning");b.innerHTML='<div class="p-6 text-center" style="color:#9CA3AF"><i class="fas fa-spinner fa-spin mr-2"></i>Generando Balance de Prueba...</div>';try{const{accounts:y}=await nt(),{transactions:A,txLines:T}=await it(),F=Object.fromEntries(A.map(w=>[w.id,w])),S=new Map(y.map(w=>[w.id,{id:w.id,code:String(w.code||""),name:String(w.name||""),level:Number(w.level||1),parent_code:String(w.parent_code||""),ownPrev:0,ownDebit:0,ownCredit:0,prev:0,debit:0,credit:0,current:0,third:new Map,children:[]}])),x=new Map;S.forEach(w=>{w.code&&x.set(w.code,w)});for(const w of T){const P=F[w.tx_id];if(!P||P.status!=="active"||!P.date)continue;const H=S.get(w.account_id);if(!H)continue;const U=String(P.date),Y=Number(w.debit||0),K=Number(w.credit||0);if(U<f?H.ownPrev+=Y-K:U>=f&&U<=d&&(H.ownDebit+=Y,H.ownCredit+=K),_){const ee=String(P.third_party_id||"NO_TERCERO"),D=((h=(g=P.expand)==null?void 0:g.third_party_id)==null?void 0:h.name)||"Sin tercero";H.third.has(ee)||H.third.set(ee,{id:ee,name:D,prev:0,debit:0,credit:0,current:0});const R=H.third.get(ee);U<f?R.prev+=Y-K:U>=f&&U<=d&&(R.debit+=Y,R.credit+=K),R.current=R.prev+R.debit-R.credit}}const C=[];S.forEach(w=>{const P=w.parent_code?x.get(w.parent_code):null;P?P.children.push(w):C.push(w)});const E=(w,P)=>w.code.localeCompare(P.code);C.sort(E),S.forEach(w=>w.children.sort(E));const I=[],N=1e-4,L=w=>{let P=w.ownPrev,H=w.ownDebit,U=w.ownCredit;for(const K of w.children){const ee=L(K);P+=ee.prev,H+=ee.debit,U+=ee.credit}const Y=P+H-U;return w.prev=P,w.debit=H,w.credit=U,w.current=Y,{prev:P,debit:H,credit:U,current:Y}};C.forEach(w=>L(w));const O=(w,P)=>{const H=[];for(const D of w.children)H.push(...O(D,P+1));if(!(Math.abs(w.prev)>N||Math.abs(w.debit)>N||Math.abs(w.credit)>N||Math.abs(w.current)>N||H.length>0))return[];const K=Number(w.level||P+1),ee={code:w.code,account:w.name,level:K,depth:P,isGroup:w.children.length>0,prev:w.prev,debit:w.debit,credit:w.credit,current:w.current,node:w};return K<=p?[ee,...H]:H};I.length=0,C.forEach(w=>I.push(...O(w,0)));const M=C.reduce((w,P)=>(w.prev+=P.prev,w.debit+=P.debit,w.credit+=P.credit,w.current+=P.current,w),{prev:0,debit:0,credit:0,current:0}),B=Et(M.prev),j=Et(M.debit),V=Et(M.credit),W=Et(M.current),J=[];for(const w of I)if(J.push({...w,thirdName:""}),_&&w.node&&w.node.third&&w.node.third.size){const P=[...w.node.third.values()].filter(H=>Math.abs(H.prev)>N||Math.abs(H.debit)>N||Math.abs(H.credit)>N||Math.abs(H.current)>N).sort((H,U)=>H.name.localeCompare(U.name));for(const H of P)J.push({code:"",account:"Detalle por tercero",level:w.level,depth:w.depth+1,isGroup:!1,prev:H.prev,debit:H.debit,credit:H.credit,current:H.current,thirdName:H.name,isThirdDetail:!0})}i=J.map(w=>({codigo:w.code,descripcion:`${"  ".repeat(w.depth)}${w.account}`,tercero:w.thirdName||"",nivel:w.level,saldo_anterior:w.prev,mov_debito:w.debit,mov_credito:w.credit,saldo_actual:w.current})),$("#btn-exp-trial")&&($("#btn-exp-trial").disabled=!J.length),$("#btn-pdf-trial")&&($("#btn-pdf-trial").disabled=!J.length),n={fromDate:f,toDate:d,includeThird:_,includeSignatures:v,displayRows:J.map(w=>({...w})),totals:{...M}};let G="";if(v){const[w,P,H,U,Y,K,ee,D]=await Promise.all([we(["representante_legal_name","legal_representative_name","rep_legal_name"]),we(["representante_legal_title","legal_representative_title","rep_legal_title"],"Representante Legal"),we(["contador_name","accountant_name"]),we(["contador_title","accountant_title"],"Contador"),we(["contador_license","accountant_license"]),we(["revisor_fiscal_name","fiscal_reviewer_name"]),we(["revisor_fiscal_title","fiscal_reviewer_title"],"Revisor Fiscal"),we(["revisor_fiscal_license","fiscal_reviewer_license"])]);G=`
          <div class="p-4 pt-2">
        ...
        <div class="overflow-x-auto p-4" style="max-height:520px">
          <table class="data-table">
            ...
            <tbody>
              ${J.length?J.map(w=>{const P=ke(w.prev),H=ke(w.debit),U=ke(w.credit),Y=ke(w.current);return`
                <tr>
    ...
    </div>`;let i=[],n=null;const r=p=>{const _=Number(String(p||"").slice(0,4)),v=Number(String(p||"").slice(5,7));if(!Number.isFinite(_)||!Number.isFinite(v)||v<1||v>12)return"";const g=new Date(_,v,0);return`${g.getFullYear()}-${String(g.getMonth()+1).padStart(2,"0")}-${String(g.getDate()).padStart(2,"0")}`},c=p=>{if(!p)return"—";const _=new Date(`${p}T00:00:00`);return Number.isNaN(_.getTime())?p:_.toLocaleDateString("es-CO",{day:"2-digit",month:"long",year:"numeric"})},l=(p,_,v,g)=>{const h=Object.fromEntries(_.map(A=>[A.id,A])),y=Object.fromEntries(p.map(A=>[A.id,0]));for(const A of v){const T=h[A.tx_id];!T||T.status!=="active"||!T.date||String(T.date)>g||(y[A.account_id]=Number(y[A.account_id]||0)+Number(A.debit||0)-Number(A.credit||0))}return y},u=(p,_,v,g)=>{const h=p.filter(x=>String(x.code||"").startsWith(g)),y=new Map(h.map(x=>{const C=Number(_[x.id]||0),E=Number(v[x.id]||0),I=g==="4"?-C:C,N=g==="4"?-E:E;return[x.id,{id:x.id,code:String(x.code||""),name:String(x.name||""),level:Number(x.level||1),parentCode:String(x.parent_code||""),ownNow:I,ownCmp:N,now:0,cmp:0,children:[]}]})),A=new Map;y.forEach(x=>{x.code&&A.set(x.code,x)});const T=[];y.forEach(x=>{const C=x.parentCode?A.get(x.parentCode):null;C?C.children.push(x):T.push(x)});const F=(x,C)=>x.code.localeCompare(C.code);T.sort(F),y.forEach(x=>x.children.sort(F));const S=x=>{let C=x.ownNow,E=x.ownCmp;for(const I of x.children){const N=S(I);C+=N.now,E+=N.cmp}return x.now=C,x.cmp=E,{now:C,cmp:E}};return T.forEach(x=>S(x)),T},b=async()=>{const p=$("#income-results");if(!p)return;const _=getInputVal("inc-month"),v=getInputVal("inc-compare-month"),g=getCheckVal("inc-show-notes"),h=getSelectVal("inc-level"),y=h==="all"?Number.POSITIVE_INFINITY:Number(h||3);if(!_||!v)return showToast("Selecciona ambos meses para el reporte comparativo.","warning");const A=r(_),T=r(v);if(!A||!T)return showToast("Mes inválido. Revisa los filtros.","warning");p.innerHTML='<div class="p-6 text-center" style="color:#9CA3AF"><i class="fas fa-spinner fa-spin mr-2"></i>Generando Estado de Resultados...</div>';try{const{accounts:F}=await nt(),{transactions:S,txLines:x}=await it(),C=l(F,S,x,A),E=l(F,S,x,T),I=u(F,C,E,"4"),N=u(F,C,E,"5"),L=u(F,C,E,"6"),O=u(F,C,E,"7");let M=1;const B=R=>{const z=[],te=X=>{const Q=[];for(const ge of X.children)Q.push(...te(ge));if(!(Math.abs(X.now)>1e-4||Math.abs(X.cmp)>1e-4)&&!Q.length)return[];const he=[];return Number(X.level||1)<=y&&he.push({note:g?String(M++):"",label:X.name,now:X.now,cmp:X.cmp}),he.push(...Q),he};R.forEach(X=>z.push(...te(X)));const Z=R.reduce((X,Q)=>X+Number(Q.now||0),0),ie=R.reduce((X,Q)=>X+Number(Q.cmp||0),0);return{detail:z,totalNow:Z,totalCmp:ie}},j=B(I),V=B(N),W=B(L),J=B(O),G=V.totalNow+W.totalNow+J.totalNow,w=V.totalCmp+W.totalCmp+J.totalCmp,P=j.totalNow-G,H=j.totalCmp-w,U=g?4:3,Y=g?'<th style="width:90px">Nota</th>':"",K=(R,q="")=>{const z=ke(R);return`<td class="text-right ${q}" style="color:${z.color}">${z.text}</td>`},ee=R=>R.detail.map(q=>`
        <tr>
    ...
    <div id="position-results" class="p-8 text-center" style="color:#9CA3AF">
      <i class="fas fa-calendar-days mr-2"></i>Selecciona mes y comparación para generar el reporte.
    </div>`;let i=[],n=null;const r=_=>{const v=Number(String(_||"").slice(0,4)),g=Number(String(_||"").slice(5,7));if(!Number.isFinite(v)||!Number.isFinite(g)||g<1||g>12)return"";const h=new Date(v,g,0),y=h.getFullYear(),A=String(h.getMonth()+1).padStart(2,"0"),T=String(h.getDate()).padStart(2,"0");return`${y}-${A}-${T}`},c=_=>{if(!_)return"—";const v=new Date(`${_}T00:00:00`);return Number.isNaN(v.getTime())?_:v.toLocaleDateString("es-CO",{day:"2-digit",month:"long",year:"numeric"})},l=(_,v,g,h)=>{const y=Object.fromEntries(v.map(T=>[T.id,T])),A=Object.fromEntries(_.map(T=>[T.id,0]));for(const T of g){const F=y[T.tx_id];!F||F.status!=="active"||!F.date||String(F.date)>h||(A[T.account_id]=Number(A[T.account_id]||0)+Number(T.debit||0)-Number(T.credit||0))}return A},u=(_,v)=>{const g=Number(_||0);return v==="asset"?g:-g},b=(_,v,g,h,y,A,T,F)=>{const x=_.filter(h),C=new Map(x.map(W=>[W.id,{id:W.id,code:String(W.code||""),name:String(W.name||""),level:Number(W.level||1),parentCode:String(W.parent_code||""),ownNow:u(v[W.id],y),ownCmp:u(g[W.id],y),now:0,cmp:0,children:[]}])),E=new Map;C.forEach(W=>{W.code&&E.set(W.code,W)});const I=[];C.forEach(W=>{const J=W.parentCode?E.get(W.parentCode):null;J?J.children.push(W):I.push(W)});const N=(W,J)=>W.code.localeCompare(J.code);I.sort(N),C.forEach(W=>W.children.sort(N));const L=W=>{let J=W.ownNow,G=W.ownCmp;for(const w of W.children){const P=L(w);J+=P.now,G+=P.cmp}return W.now=J,W.cmp=G,{now:J,cmp:G}};I.forEach(W=>L(W));let O=T;const M=W=>{const J=[];for(const H of W.children)J.push(...M(H));if(!(Math.abs(W.now)>1e-4||Math.abs(W.cmp)>1e-4||J.length>0))return[];const P=[];return Number(W.level||1)<=F&&P.push({note:A?String(O++):"",label:W.name,now:W.now,cmp:W.cmp}),P.push(...J),P},B=I.flatMap(W=>M(W)),j=I.reduce((W,J)=>W+J.now,0),V=I.reduce((W,J)=>W+J.cmp,0);return{detail:B,totalNow:j,totalCmp:V,nextNote:O}},f=async()=>{const _=$("#position-results");if(!_)return;const v=getInputVal("pos-month"),g=getInputVal("pos-compare-month"),h=getCheckVal("pos-show-notes"),y=getSelectVal("pos-level"),A=y==="all"?Number.POSITIVE_INFINITY:Number(y||3);if(!v||!g)return showToast("Selecciona ambos meses para el reporte comparativo.","warning");const T=r(v),F=r(g);if(!T||!F)return showToast("Mes inválido. Revisa los filtros.","warning");_.innerHTML='<div class="p-6 text-center" style="color:#9CA3AF"><i class="fas fa-spinner fa-spin mr-2"></i>Generando Estado de Situación Financiera...</div>';try{const{accounts:S}=await nt(),{transactions:x,txLines:C}=await it(),E=l(S,x,C,T),I=l(S,x,C,F);let N=1;const L=b(S,E,I,D=>String(D.code||"").startsWith("11"),"asset",h,N,A);N=L.nextNote;const O=b(S,E,I,D=>String(D.code||"").startsWith("1")&&!String(D.code||"").startsWith("11"),"asset",h,N,A);N=O.nextNote;const M=b(S,E,I,D=>String(D.code||"").startsWith("21"),"liability",h,N,A);N=M.nextNote;const B=b(S,E,I,D=>String(D.code||"").startsWith("2")&&!String(D.code||"").startsWith("21"),"liability",h,N,A);N=B.nextNote;const j=b(S,E,I,D=>String(D.code||"").startsWith("3"),"equity",h,N,A),V=L.totalNow+O.totalNow,W=L.totalCmp+O.totalCmp,J=M.totalNow+B.totalNow,G=M.totalCmp+B.totalCmp,w=J+j.totalNow,P=G+j.totalCmp,H=h?4:3,U=h?'<th style="width:90px">Nota</th>':"",Y=(D,R="")=>{const q=ke(D),z=`color:${q.color}`;return`<td class="text-right ${R}" style="${z}">${q.text}</td>`},K=D=>D.detail.map(R=>`
        <tr>
      ...
      <div class="bg-white rounded-2xl border overflow-hidden" style="border-color:#F0F0F0">
        <div class="overflow-x-auto" style="max-height: calc(100vh - 290px)">
          <table class="data-table" id="mov-table">
            ...
            <tbody>
              ${_.length?_.map(x=>{var C,E,I,N;return`
                <tr data-bank-id="${esc(x.bank_account_id)}" data-mov-id="${esc(x.id)}" data-reconciled="${x.reconciled?"1":"0"}" data-date="${esc(x.date)}">
                  ...
                  <td>${esc(x.date)}</td>
                  <td>${esc(((E=(C=x.expand)==null?void 0:C.bank_account_id)==null?void 0:E.bank)||"")} - ${esc(((N=(I=x.expand)==null?void 0:I.bank_account_id)==null?void 0:N.number)||"")}</td>
...
`)}`);const b=((d=a.find(m=>!!m.employee_id))==null?void 0:d.employee_id)||"";return(await API.createTransaction({tx_type_id:s.id,date:t.date_to||todayStr(),description:`Nómina ${t.name}`,third_party_id:b||void 0},n)).id}async function ei(e=[]){var t,a,o,s,i,n,r,c;try{const[{row:l,config:u},b,f]=await Promise.all([da(),pb.listAll("accounts",{filter:"active=true",sort:"code"}),pb.listAll("third_parties",{filter:"active=true",sort:"name"})]);if(!b.length)return showToast("No hay cuentas activas para mapear.","warning");const d={rowId:(l==null?void 0:l.id)||"",config:Ot(u)},m=`<option value="">Selecciona cuenta...</option>${b.map(S=>`<option value="${esc(S.id)}">${esc(S.code)} - ${esc(S.name)}</option>`).join("")}`,p=`<option value="">Selecciona categoría...</option>${Object.keys(Ca).map(S=>`<option value="${esc(S)}">${esc(Ca[S])}</option>`).join("")}`,_=()=>`<option value="">Selecciona grupo...</option>${(d.config.employee_groups||[]).map(S=>`<option value="${esc(S.id)}">${esc(S.name)}</option>`).join("")}`;openModal("Configuración Contable de Nómina",`
        ...
        <div class="overflow-x-auto">
          <table class="data-table text-sm">
            ...
            <tbody id="nom-emp-rules-body"></tbody>
             ...
             </div>`),h.innerHTML=y.length?y.map(T=>{const F=Je(l.config,T.id),S=fo(l.config,T.id),x=sa(F),C=F.group_id?f[F.group_id]||"Grupo no encontrado":"Sin grupo";return`<tr${x?"":' style="background:#FFF7ED"'}>
      ...
      <div class="bg-white rounded-2xl border overflow-hidden" style="border-color:#F0F0F0">
       ...
       </tr>`,j=v.hasBreakdown?v.breakdown.filter(w=>w.hours>0).map(w=>B(`${w.label} (${w.hours} h)`,w.amount)).join(""):d.overtime?B("Horas extra / recargos",d.overtime||0):"",V=Oe.filter(w=>(h[w]||0)>0).map(w=>{var P;return B(((P=Ue[w])==null?void 0:P.label)||w,h[w])}).join(""),W=Be.filter(w=>(h[w]||0)>0).map(w=>{var P;return B(((P=Ue[w])==null?void 0:P.label)||w,h[w])}).join(""),J=`<!DOCTYPE html>
<html lang="es">
<head>
  <meta charset="UTF-8">
...
`+t],{type:"text/csv;charset=utf-8;"}),o=URL.createObjectURL(a),s=document.createElement("a");s.href=o,s.download="plantilla_unidades_copropiedades.csv",s.click(),URL.revokeObjectURL(o)}function $i(e,t,a){var h,y;const o=ve(e,["codigo","code","unidad","unit_code"]),s=ve(e,["nombre","name","descripcion"]),i=ve(e,["tipo","unit_type","tipo_unidad"])||"APARTAMENTO",n=ve(e,["torre","tower"]),r=ve(e,["apartamento","apto","apartment"]),c=Pt(ve(e,["coef_participacion","coef","coeficiente"])),l=Pt(ve(e,["cuota_admin","admin_fee","cuota_administracion"])),u=Pt(ve(e,["area_m2","area"])),b=ve(e,["doc_propietario","owner_doc","documento_propietario"]),f=ve(e,["tipo_doc_propietario","owner_doc_type","doc_type_propietario"]).toUpperCase(),d=ve(e,["activo","active","estado"]),m=ve(e,["notas","nota","notes"]);if(!o)return{ok:!1,error:"Falta el código de la unidad"};if(!s)return{ok:!1,error:`Falta el nombre para la unidad ${o}`};const p=new Set(["APARTAMENTO","PARQUEADERO","DEPOSITO","LOCAL","CASA","OFICINA","OTRO"]),_=String(i).toUpperCase();if(!p.has(_))return{ok:!1,error:`Tipo inválido en ${o}: ${i}`};if(c<0||c>100)return{ok:!1,error:`Coeficiente fuera de rango (0-100) en ${o}`};if(l<0)return{ok:!1,error:`Cuota administración negativa en ${o}`};if(u<0)return{ok:!1,error:`Área negativa en ${o}`};let v=null;if(b){const A=Ft(b);if(f&&(v=((h=a.get(`${f}|${A}`))==null?void 0:h.id)||null),v||(v=((y=t.get(A))==null?void 0:y.id)||null),!v)return{ok:!1,error:`No existe tercero propietario con documento ${b}`}}const g=!/^(no|0|false|inactiva|inactivo)$/i.test(d);return{ok:!0,payload:{code:o,name:s,unit_type:_,tower:n,apartment:r,coef_participacion:c,admin_fee:l,area_m2:u,owner_id:v,notes:m,active:g}}}async function wi(){var u,b,f;if(!can("canWrite"))return showToast("No tienes permisos para importar unidades","error");if(Tt)return showToast("Importación en curso, espera...","warning");Ht('<i class="fas fa-building-user mr-2" style="color:#0E7490"></i>Importar Unidades Copropiedades',`
    ...
    <div id="ph-tab-content"></div>`;const o=e.querySelector("#ph-tab-content");function s(i){e.querySelectorAll(".tab-btn").forEach(n=>n.classList.toggle("active",n.dataset.tab===i)),i==="facturacion"&&va(o),i==="cartera"&&or(o),i==="unidades"&&Ja(o),i==="reservas"&&ca(o),i==="pqrs"&&$s(o),i==="config"&&et(o)}e.querySelectorAll(".tab-btn").forEach(i=>i.addEventListener("click",()=>s(i.dataset.tab))),s(t)}async function va(e){var t,a,o,s,i,n;e.innerHTML='<div class="p-6 text-center" style="color:#9CA3AF"><i class="fas fa-spinner fa-spin mr-2"></i>Cargando...</div>';try{const r=_t(),c=pb.escapeFilterValue(r),[l,u]=await Promise.all([API.getPhInvoices({filter:`period="${c}"`,perPage:200}),API.getPhInvoices({filter:"",perPage:1})]),b=l.items||[],f=u.totalItems||0,d=b.filter(v=>v.status==="posted").length,m=b.filter(v=>v.status==="paid").length,p=b.filter(v=>v.status==="draft").length,_=b.reduce((v,g)=>v+(g.total||0),0);e.innerHTML=`
      ...
      <div id="ph-cartera-detalle" class="cartera-tab-content" style="display:none">
        <div class="bg-white rounded-2xl border overflow-hidden" style="border-color:#F0F0F0">
          ...
          <div class="overflow-x-auto">
            ...
            <tr><td colspan="2" class="text-center py-4" style="color:#9CA3AF">No hay saldos abiertos para los filtros seleccionados.</td></tr>`,document.getElementById("ph-cartera-resumen-tfoot").innerHTML="",document.getElementById("ph-cartera-bal-meta").innerHTML='<i class="fas fa-info-circle mr-1"></i>Sin datos de saldo abierto.',r=null;const C=document.getElementById("ph-cartera-pdf-bal");C&&(C.disabled=!0)}else{const C=new Map;for(const j of h){const V=String(j.concepto||"Concepto").trim()||"Concepto",W=v(V);C.has(W)||C.set(W,V)}const E=[...C.entries()].map(([j,V])=>({id:j,label:V})).sort((j,V)=>j.label.localeCompare(V.label,"es")),I=new Map;for(const j of h){const V=[j.propertyCode,j.propertyName].filter(Boolean).join(" - ")||"Unidad",W=`${j.propertyId}|${V}`;I.has(W)||I.set(W,{unidad:V,byConcept:{},totalGeneral:0});const J=I.get(W),G=v(j.concepto||"Concepto");J.byConcept[G]=(J.byConcept[G]||0)+Number(j.amount||0),J.totalGeneral+=Number(j.amount||0)}const N=[...I.values()].sort((j,V)=>j.unidad.localeCompare(V.unidad,"es")),L={};let O=0;for(const j of N){O+=Number(j.totalGeneral||0);for(const V of E)L[V.id]=(L[V.id]||0)+Number(j.byConcept[V.id]||0)}const M=new Set(h.map(j=>String(j.invoiceId||""))).size;document.getElementById("ph-cartera-bal-meta").innerHTML=`Unidades: <strong>${fmtN(N.length)}</strong> · Conceptos: <strong>${fmtN(E.length)}</strong> · Documentos: <strong>${fmtN(M)}</strong> · Saldo abierto: <strong>${fmt(O)}</strong>`,_&&(_.innerHTML=`
              <col style="width:260px">
          ...
          <tr><td colspan="2" class="text-center py-4" style="color:#EF4444">${esc(F.message)}</td></tr>`,document.getElementById("ph-cartera-resumen-tfoot").innerHTML="",i(null),r=null;const S=document.getElementById("ph-cartera-pdf-bal");S&&(S.disabled=!0)}try{if(h.length===0){document.getElementById("ph-cartera-detalle-tbody").innerHTML=`
            ...
            </tr>`,c={rows:x,totals:E};const I=document.getElementById("ph-cartera-pdf-aging");I&&(I.disabled=!1)}}catch(F){console.error(F),document.getElementById("ph-cartera-detalle-tbody").innerHTML=`
     ...
     </button>`),setTimeout(()=>{var u;(u=document.getElementById("pq-save-btn"))==null||u.addEventListener("click",async()=>{var f,d,m,p,_,v,g,h,y;const b=document.getElementById("pq-save-btn");b&&(b.disabled=!0,b.textContent="Guardando...");try{if(o){const A=(f=document.getElementById("pq-status"))==null?void 0:f.value,T=((d=document.getElementById("pq-response"))==null?void 0:d.value)||"",F=((m=document.getElementById("pq-assigned"))==null?void 0:m.value)||"",S=((p=document.getElementById("pq-priority"))==null?void 0:p.value)||"media",x={status:A,response:T,assigned_to:F,priority:S};(A==="closed"||A==="resolved")&&(x.closed_at=new Date().toISOString().replace("T"," ").slice(0,19)),await pb.update("ph_pqrs",o.id,x),await API.logAudit("UPDATE","PhPqr",o.id,`PQR ${o.number} → ${A}`),showToast("PQR actualizada.","success")}else{const A=(((_=document.getElementById("pq-subject"))==null?void 0:_.value)||"").trim(),T=(((v=document.getElementById("pq-desc"))==null?void 0:v.value)||"").trim(),F=((g=document.getElementById("pq-type"))==null?void 0:g.value)||"PETICION",S=((h=document.getElementById("pq-priority"))==null?void 0:h.value)||"media",x=((y=document.getElementById("pq-prop"))==null?void 0:y.value)||null;if(!A){showToast("El asunto es obligatorio.","warning"),b&&(b.disabled=!1,b.textContent="Crear PQR");return}if(!T){showToast("La descripción es obligatoria.","warning"),b&&(b.disabled=!1,b.textContent="Crear PQR");return}const C=await API.nextPhPqrNumber(),E=await pb.create("ph_pqrs",{number:C,subject:A,description:T,pqrs_type:F,priority:S,property_id:x||null,status:"open",opened_at:new Date().toISOString().replace("T"," ").slice(0,19)});await API.logAudit("CREATE","PhPqr",E.id,`Nueva PQR ${C} — ${A}`),showToast("PQR creada correctamente.","success")}closeModal(),$s(t)}catch(A){showToast(A.message||"Error.","error"),b&&(b.disabled=!1,b.textContent=o?"Actualizar":"Crear PQR")}},{once:!0})},50)}async function et(e){var t,a,o,s,i;e.id=e.id||"ph-config-container",e.innerHTML='<div class="p-6 text-center" style="color:#9CA3AF"><i class="fas fa-spinner fa-spin mr-2"></i>Cargando...</div>';try{const[n,r,c,l,u,b]=await Promise.all([API.getPhBillingConcepts(!1),API.getPhCommonAreas(!1),API.getSetting("ph_config_v1"),API.getAccounts(!0),API.getPhProperties(!0),API.getPhIndividualCharges({filter:""}).catch(()=>({items:[]}))]),f=((b==null?void 0:b.items)||[]).slice().sort((x,C)=>{const E=String((x==null?void 0:x.name)||(x==null?void 0:x.description)||"").toLowerCase(),I=String((C==null?void 0:C.name)||(C==null?void 0:C.description)||"").toLowerCase();return E.localeCompare(I)}),d=new Map((u||[]).map(x=>[x.id,x]));let m={};try{m=c?JSON.parse(c):{}}catch{m={}}const p=m.cxc_code||"130505",_=m.income_code||"413505",v=m.late_fee_income_code||_,g=m.anticipo_account_code||"",h=(l||[]).filter(x=>x.active!==!1&&Number(x.level||0)>=3).sort((x,C)=>String(x.code||"").localeCompare(String(C.code||""))),y=new Map(h.map(x=>[String(x.code||""),x])),A=h.filter(x=>String(x.code||"").startsWith("1")),T=h.filter(x=>String(x.code||"").startsWith("4")),F=h.filter(x=>String(x.code||"").startsWith("2")),S=(x,C="")=>{const E=String(C||""),I=x.some(L=>String(L.code||"")===E);return`${E&&!I?`<option value="${esc(E)}" selected>${esc(E)} — (No encontrada en PUC activo)</option>`:""}<option value="">— Seleccionar cuenta —</option>${x.map(L=>`<option value="${esc(L.code)}"${String(L.code||"")===E?" selected":""}>${esc(L.code)} — ${esc(L.name||"")}</option>`).join("")}`};e.innerHTML=`
...
<div class="no-print">
  ...
  `,document.querySelectorAll(".teso-tab").forEach(a=>{a.addEventListener("click",o=>{const s=o.target.dataset.target;Ss(s)})}),Ss("dashboard"))}function Ss(e){document.querySelectorAll(".teso-tab").forEach(a=>{a.dataset.target===e?(a.classList.add("text-blue-600","border-blue-600"),a.classList.remove("text-gray-500","border-transparent")):(a.classList.remove("text-blue-600","border-blue-600"),a.classList.add("text-gray-500","border-transparent"))});const t=document.getElementById("teso-content");e==="dashboard"&&vl(),e==="recaudos"&&Pa(t,"RC"),e==="pagos"&&Pa(t,"CE")}window.registerModule&&window.registerModule("tesoreria",br);window.showTesoreriaScreen=br;window.openRecaudoModal=bl;window.openPagoModal=gl;window.openTesoreriaConfigModal=hl;window._toggleModalManualMode=pl;window._saveTransaccionTeso=ml;window._updateMontoIndicator=ul;window._changeTesoOrigen=_changeTesoOrigen;function yl(){const e=window.XLSX;if(!e){Te("Librería XLSX no cargada","error");return}const t=e.utils.aoa_to_sheet([["codigo_unidad","fecha","valor","referencia","observaciones"],["A101","2026-05-16",45e4,"TRANSF-9821","Pago mayo"],["B202","2026-05-16",38e4,"","Pago cuota ordinaria"],["C303","2026-05-16",52e4,"CHQ-4455",""]]);t["!cols"]=[{wch:16},{wch:14},{wch:12},{wch:18},{wch:24}];const a=e.utils.book_new();e.utils.book_append_sheet(a,t,"Recaudos"),e.writeFile(a,"plantilla_recaudos_ph.xlsx")}async function _l(){const e=Fe(),t=await e.listAll("bank_accounts",{expand:"account_id",filter:"active=true",sort:"name"});if(!t.length){Te("No hay cuentas bancarias activas","warning");return}let a=[];const s=`<div style="font-family:'Segoe UI',sans-serif">
    ...
    <button class="btn btn-primary hidden" id="mass-rc-btn-next"></button>`,!0),setTimeout(()=>{const n=document.getElementById("mass-rc-drop"),r=document.getElementById("mass-rc-file"),c=document.getElementById("mass-rc-btn-next"),l=f=>{n&&(n.style.borderColor=f?"#1D6F42":"#D1D5DB",n.style.background=f?"#ECFDF5":"#FAFAFA")};n==null||n.addEventListener("click",()=>r==null?void 0:r.click()),n==null||n.addEventListener("dragover",f=>{f.preventDefault(),l(!0)}),n==null||n.addEventListener("dragleave",()=>l(!1)),n==null||n.addEventListener("drop",f=>{var m,p;f.preventDefault(),l(!1);const d=(p=(m=f.dataTransfer)==null?void 0:m.files)==null?void 0:p[0];d&&u(d)}),r==null||r.addEventListener("change",()=>{var f;(f=r.files)!=null&&f[0]&&u(r.files[0])}),c==null||c.addEventListener("click",()=>b());async function u(f){var L,O,M,B,j;if(f.size>8*1024*1024){Te("El archivo supera 8 MB","error");return}const d=window.XLSX;if(!d){Te("Librería XLSX no cargada","error");return}const m=document.getElementById("mass-rc-cuenta");if(!(m!=null&&m.value)){Te("Selecciona un método de pago primero","warning");return}const p=m.value,_=((O=(L=m.options[m.selectedIndex])==null?void 0:L.dataset)==null?void 0:O.account)||"",v=d.read(await f.arrayBuffer(),{type:"array",cellDates:!0}),g=v.Sheets[v.SheetNames[0]],h=d.utils.sheet_to_json(g,{defval:""}),y=V=>String(V||"").toLowerCase().normalize("NFD").replace(/[\u0300-\u036f]/g,"").replace(/\s+/g,"_").trim(),A=h.map(V=>{const W={};return Object.entries(V).forEach(([J,G])=>{W[y(J)]=G}),W}).filter(V=>V.codigo_unidad||V.codigo||V.unidad);if(!A.length){Te("No se encontraron filas con datos","warning");return}const T=await e.listAll("ph_properties",{filter:"active=true",expand:"owner_id",sort:"code"}),F=new Map(T.map(V=>[String(V.code||"").trim().toUpperCase(),V])),x=((M=(await e.listAll("transaction_types",{filter:'code="RC"'}))[0])==null?void 0:M.id)||"";a=A.map((V,W)=>{var D,R;const J=String(V.codigo_unidad||V.codigo||V.unidad||"").toUpperCase().trim(),G=V.fecha||V.date||"",w=V.valor||V.value||V.monto||0,P=String(V.referencia||V.reference||"").trim(),H=String(V.observaciones||V.obs||"").trim();let U="";if(G instanceof Date)U=G.toISOString().slice(0,10);else{const q=String(G).trim();if(/^\d{4}-\d{2}-\d{2}$/.test(q))U=q;else if(/^\d{1,2}\/\d{1,2}\/\d{4}$/.test(q)){const[z,te,Z]=q.split("/");U=`${Z}-${te.padStart(2,"0")}-${z.padStart(2,"0")}`}else{const z=new Date(q);isNaN(z.getTime())||(U=z.toISOString().slice(0,10))}}const Y=Number(String(w).replace(/[^0-9.]/g,""))||0,K=F.get(J),ee=[];return J?K?(D=K.expand)!=null&&D.owner_id||ee.push("Sin propietario"):ee.push(`Unidad "${J}" no encontrada`):ee.push("Falta código"),U||ee.push("Fecha inválida"),Y<=0&&ee.push("Valor debe ser > 0"),{rowNo:W+2,codigo:J,fecha:U,valor:Y,ref:P,obs:H,prop:K,txTypeId:x,bankAccountId:p,cuentaAccId:_,owner:((R=K==null?void 0:K.expand)==null?void 0:R.owner_id)||null,ok:ee.length===0,errors:ee}}),(B=document.getElementById("mass-rc-step1"))==null||B.classList.add("hidden"),(j=document.getElementById("mass-rc-step2"))==null||j.classList.remove("hidden");const C=a.filter(V=>V.ok).length,E=a.length-C,I=document.getElementById("mass-rc-badge");I&&(I.innerHTML=`<span style="color:${E>0?"#B91C1C":"#166534"}">${a.length} filas · ${C} válidas${E>0?" · "+E+" con error":""}</span>`);const N=document.getElementById("mass-rc-tbody");N&&(N.innerHTML=a.map(V=>{var W;return`<tr style="background:${V.ok?"":"#FFF7F7"}">
      ...
      </tr>`}).join("")),C>0&&(c.classList.remove("hidden"),c.innerHTML=`<i class="fas fa-bolt mr-1"></i>Procesar ${C} recaudo(s)`)}async function b(){var A,T,F,S;const f=a.filter(x=>x.ok);if(!f.length)return;(A=document.getElementById("mass-rc-step2"))==null||A.classList.add("hidden"),(T=document.getElementById("mass-rc-step3"))==null||T.classList.remove("hidden"),c.classList.add("hidden");const d=document.getElementById("mass-rc-bar"),m=document.getElementById("mass-rc-status"),p=document.getElementById("mass-rc-detail");let _=0,v=0;const g=[];for(let x=0;x<f.length;x++){const C=f[x];d&&(d.style.width=`${Math.round(x/f.length*100)}%`),m&&(m.textContent=`Procesando ${x+1} de ${f.length}...`),p&&(p.textContent=`Unidad ${C.codigo} — ${xe(C.valor)}`);try{await e.create("transactions",{tx_type_id:C.txTypeId,number:`RC-MASIVO-${Date.now()}-${x}`,date:C.fecha,third_party_id:C.owner.id,description:C.obs||`Recaudo carga masiva${C.ref?" Ref: "+C.ref:""}`,status:"active",teso_mode:"auto",teso_params:JSON.stringify({third_party_id:C.owner.id,ph_property_id:C.prop.id,amount:C.valor,contrapartida_account_id:C.cuentaAccId,reglas:{primeroVencido:!0,primeroMora:!0}})}),_++}catch(E){v++,g.push(`Unidad ${C.codigo}: ${E.message}`)}}d&&(d.style.width="100%"),m&&(m.textContent="Proceso completado"),p&&(p.textContent=""),(F=document.getElementById("mass-rc-spin"))==null||F.classList.add("hidden"),(S=document.getElementById("mass-rc-done-icon"))==null||S.classList.remove("hidden");const h=document.getElementById("mass-rc-result");h&&(h.classList.remove("hidden"),h.innerHTML=`<div class="rounded-xl p-4" style="background:#F9FAFB;border:1px solid #E5E7EB">
