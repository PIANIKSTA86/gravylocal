...
<Invoice xmlns="urn:oasis:names:specification:ubl:schema:xsd:Invoice-2" xmlns:cac="urn:oasis:names:specification:ubl:schema:xsd:CommonAggregateComponents-2" xmlns:cbc="urn:oasis:names:specification:ubl:schema:xsd:CommonBasicComponents-2">
  ...
  <cac:AccountingSupplierParty>
    <cac:Party>
      ...
      <cac:PhysicalLocation>
        <cac:Address>
          <cbc:CityCode>11001</cbc:CityCode>
  ...
  <cac:AccountingCustomerParty>
    <cac:Party>
      ...
      <cac:PhysicalLocation>
        <cac:Address>
          <cbc:CityCode>76001</cbc:CityCode>
...
let fnCode = orchestratorCode.substring(fnStart, fnEnd);

fnCode = fnCode.replace(
  "const drawMetaLine = (label, val, yPos) => {",
  "const drawMetaLine = (label, val, yPos) => { console.log('META_LINE:', label, '=>', val);"
);
fnCode = fnCode.replace(
  "const drawInfoLine = (label, val, xOffset, yOffset) => {",
  "const drawInfoLine = (label, val, xOffset, yOffset) => { console.log('INFO_LINE (x=' + xOffset + '):', label, '=>', val);"
);
