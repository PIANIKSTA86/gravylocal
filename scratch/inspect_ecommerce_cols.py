...
cols = ['products', 'inventory_stock', 'sales_orders', 'sales_order_lines']
db_path = r'c:\Users\JULIAN\Desktop\GravyLocal2.0\pb_data\data.db'
conn = sqlite3.connect(db_path)
cursor = conn.cursor()

for col_name in cols:
    cursor.execute("SELECT name, listRule, viewRule, createRule, updateRule, deleteRule, fields FROM _collections WHERE name=?", (col_name,))
    row = cursor.fetchone()
    if row:
        ...
        try:
            ...
            for f in fields:
                ...
                print(f"  - {name_f} ({type_f}){req}{hidden}")
