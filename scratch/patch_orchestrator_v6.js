...
const newElseBlock = `      } else {
        ...
        // Document Type Title + FE - Number on same baseline
        doc.fillColor('#000000').font('Helvetica-Bold').fontSize(11.5);
        const docNumOnly = docId.replace(/^[A-Za-z\\-]+/g, '');
        doc.text(\`\${docTypeLabel.toUpperCase()}  FE - \${docNumOnly}\`, rx, ry, { width: rw });
        const docTypeHeight = doc.heightOfString(\`\${docTypeLabel.toUpperCase()}  FE - \${docNumOnly}\`, { width: rw });
...
if (startIdx === -1) {
  console.error("ERROR: startTag not found!");
  process.exit(1);
}

// Find closing block: doc.end(); followed by } and then the function close block
const sub = content.substring(startIdx);
const endPattern = /doc\.end\(\);\s*\}/;
const match = sub.match(endPattern);

if (!match) {
  console.error("ERROR: closing block not found in sub!");
  process.exit(1);
}

const endIdx = startIdx + match.index + match[0].length;

const newContent = content.substring(0, startIdx) + newElseBlock + content.substring(endIdx);
fs.writeFileSync(path, newContent, 'utf8');
console.log("SUCCESS: Replaced standard layout block inside generateInvoicePdf successfully!");
