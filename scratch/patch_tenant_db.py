...
if row:
    ...
    for field in fields:
        if field.get('name') == 'enabled' and field.get('required') is True:
            ...
            modified = True
            print("Found 'enabled' field in Tenant DB. Changed required to False.")
            
    if modified:
        new_fields_json = json.dumps(fields)
        cursor.execute("UPDATE _collections SET fields = ? WHERE name = 'licenses'", (new_fields_json,))
        conn.commit()
        print("Tenant Database schema successfully patched in SQLite _collections table!")
    else:
        print("'enabled' field in Tenant DB was already required: False or not found.")
else:
    print("Collection 'licenses' not found in Tenant DB.")
