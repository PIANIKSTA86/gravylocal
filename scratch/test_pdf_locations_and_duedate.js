...
const scriptToRun = orchestratorCode + `\n
...
fs.writeFileSync(path.join(__dirname, 'runner_test.js'), scriptToRun, 'utf8');
try {
  const { execSync } = require('child_process');
  const out = execSync('node scratch/runner_test.js', { encoding: 'utf8' });
  console.log("Test Output:", out);
} catch (e) {
  console.error("Execution failed:", e.message);
} finally {
  if (fs.existsSync(path.join(__dirname, 'runner_test.js'))) {
    fs.unlinkSync(path.join(__dirname, 'runner_test.js'));
  }
}
