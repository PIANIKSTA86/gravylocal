...
async function run() {
  ...
  console.log(`Testing resend email for Invoice ID: ${invoice.id}, Number: ${invoice.number}, Tx ID: ${txId}`);

  // Now post to /api/dian/resend-email
  const resendRes = await fetch(`${pbUrl}/api/dian/resend-email`, {
    method: 'POST',
    headers: {
  ...
  try {
    ...
    console.log("Response text (not JSON):", await resendRes.text());
  }
}
