# PowerShell Script to cleanly uninstall GravyLocal 2.0 Portable background services.
...
Write-Host "   GRAVY v2.0 - Desinstalador Portable              " -ForegroundColor $red
Write-Host "===================================================" -ForegroundColor $cyan
Write-Host ""
...
Write-Host "    [OK] Tarea '$taskName' eliminada con exito." -ForegroundColor $green
...
if (Test-Path "$rootPath\kill.ps1") {
    ...
    Write-Host "    [OK] Procesos en puertos finalizados." -ForegroundColor $green
} else {
    Write-Host "    [X] No se encontro kill.ps1, saltando detencion de puertos." -ForegroundColor $yellow
}
...
foreach ($port in $ports) {
    ...
    Write-Host "    [OK] Regla de firewall para puerto $port eliminada." -ForegroundColor $green
}
...
foreach ($sc in $shortcuts) {
    ...
    if (Test-Path $filePath) {
        ...
        Write-Host "    [OK] Eliminado acceso directo: $sc" -ForegroundColor $green
    }
}

Write-Host ""
Write-Host "===================================================" -ForegroundColor $cyan
Write-Host "   DESINSTALACION PORTABLE COMPLETADA               " -ForegroundColor $green
Write-Host "===================================================" -ForegroundColor $cyan
Write-Host "Los servicios automaticos han sido completamente removidos." -ForegroundColor $white
Write-Host ""
pause
